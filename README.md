...
Vue's built-in `v-show` works on `<Box>` roots and keeps their ordinary component subtree mounted while removing hidden layout content from terminal layout, paint, targeted focus availability, geometry, and Fullscreen hit testing. `Static` is not a layout node: ancestor or direct `v-show` leaves its mounted history behavior unchanged. `v-if` remains the lifecycle-owning choice. Direct `v-show` on `Text` is not supported.
