...
- **`@vue-tui/runtime`** — the engine and only the public primitives whose correct behavior requires ownership of the terminal, renderer tree, accepted layout or paint, input protocol, lifecycle, or terminal resources. The implemented minimum-foundation candidate includes `createApp`, `renderToString`, `Box`, `Text`, `useApp`, `useInput`, `useStdin`, minimal focus ownership, direct layout and Box facts, `/inline`'s `Static`, the narrow `/devtools` and `/testing` integration seams, and package metadata resolution. Rich session, routing, paint geometry, caret, pointer, selection, clipboard, and output-coordination state remains internal. Runtime depends on nothing else in the family. `Static` remains Runtime work because its one-attempt acceptance and stream-commit ownership are renderer/output mechanics even though Vue owns collection iteration and its authoring path is surface-specific; its exact live ordering, hidden-ancestor behavior, placement, nesting, and Static-specific failure timing remain Open. A future physical-interaction feature may add a narrow Runtime operation when a concrete task proves it, without exporting the retained broad internals.
