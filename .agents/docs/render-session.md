...
> **Status:** completed F1.3 public-API proposal, F1.4 private live-source implementation, F1.5 deterministic-test/string-host authority, F1.6 Inline ownership, and F1.7 lifecycle behavior, unstamped. F1.8 publication is Active. This record selects one target readonly session shape and maps it to every accepted host class. The public projection remains intentionally unexported until the publication gates are complete.
...
This distinction follows Ratatui's useful separation between backend terminal size and the current frame render area. `renderToString()` now uses the supplied document columns for both layout and `useWindowSize()` without reading the developer's `process.stdout` or registering a process resize listener.
...
F1.4 selects `liveUpdates?: boolean` as the public output-cadence override and removes own `interactive` keys synchronously. CI and stdout TTY state choose only its omission default. The resolved live surface, rather than the requested mode or another boolean, now drives alternate-screen entry, fixed viewport layout, transcript fallback, hit-map construction, update cadence, and resize dimensions. The internal service is provided before root setup, has stable identity, replaces dimensions atomically, and stops updating after teardown. F1.5 extends that service to both string presentations and deterministic modeled hosts, so `useWindowSize()` and `useIsScreenReaderEnabled()` derive from the provided host rather than process-global state.
