...
Raw-input support remains an independent internal resolver input, not an F1.3 public field. Its current TTY marker cannot answer whether an application can safely register semantic input before acquisition. F3's accepted target removes `useStdin().isRawModeSupported` and makes semantic-route demand own acquisition while treating non-TTY routes as unavailable; the final input API still decides how an application queries that result. F1 only ensures output resolution never decides input availability implicitly.
...
| `useStdin()`                        | Retain only raw stdin escape hatch | Its `stdin` field exposes the actual mounted stream without framework event or safe routing-composition guarantees. The accepted F3 target removes `setRawMode` and `isRawModeSupported`; semantic routes own acquisition. |
