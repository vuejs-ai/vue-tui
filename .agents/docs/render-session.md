...
| `useStdin()`                        | Retain raw stdin escape hatch    | Its `stdin` field exposes the actual mounted stream without framework event or safe routing-composition guarantees. F3 still decides acquisition, availability, and the remaining raw-mode operations.        |
