...
Raw-input support remains an independent internal resolver input, not an F1.3 public field. Its TTY marker cannot answer whether an application can safely register semantic input before acquisition. F3 removed `useStdin().isRawModeSupported`; semantic-route demand now owns acquisition while non-TTY routes are unavailable. The final input API still decides how an application queries that result. F1 only ensures output resolution never decides input availability implicitly.
...
| `useStdin()`                        | Retain only raw stdin escape hatch          | Its sole `stdin` field exposes the actual mounted stream without framework event or safe routing-composition guarantees. F3 removed public raw-mode operations; semantic routes own acquisition.                                                                                                                                                                |
| `useStdout()` / `useStderr()`       | Retained; Runtime closure refined `write()` | Coordinated TTY writes accept geometry-safe styled lines, redirected/non-TTY output stays byte-exact, and the returned streams remain raw escape hatches. `CoordinatedWriteResult` distinguishes accepted writable output, accepted backpressure, and blocked non-acceptance without queueing caller bytes. These operations are not copied into session facts. |
...
A final-stream test uses the real final-output policy: new `/inline` Static output is visible immediately and only the latest dynamic frame appears at clean teardown; fatal teardown suppresses that retained success and writes the error to stderr. A Fullscreen test may model effective Fullscreen and its capabilities without writing to the developer's real alternate screen; mounting Static on that effective visual surface rejects through the production path. `terminal.suspend()` releases modeled terminal ownership and `terminal.resume()` refreshes dimensions, re-establishes and repaints the same resolved live-update surface—including a row-unbounded live stream—before input is reacquired; the deterministic control does not stop the JavaScript event loop. On a live-update surface, resizing updates the modeled streams, emulator, authoritative session, and resulting frame in order; a final-output surface retains production's lack of a runtime resize handler even though the test emulator itself can be resized. `unmount()` keeps the restored screen inspectable, idempotent `dispose()` releases every host resource and disables further host operations, and automatic cleanup calls that same disposer; console patching stays disabled so overlapping deterministic renders never mutate process-global console methods. Application code behaves from the modeled production facts and cannot branch on a public test host.
