...
The original measurement adapter subscribed to layout for the current resolved host and reset its scalar Yoga metrics synchronously when that host disappeared. F5 removed that API and the imperative Yoga read, then temporarily published `useElementGeometry()` on the same F2 attachment identity. Path 1 keeps its authoritative accepted-paint source private and publishes `useBoxSize()` only for a direct `Box` in the current app. The public projection contains the last accepted full width and height or `null`; it never exposes renderer nodes, coordinates, fragments, or private caret slots.
...
The F2 migrations did not freeze either public API. F5 replaced `useBoxMetrics()` with the experimental `useElementGeometry()` and removed `measureElement()` without aliases; Path 1 then replaced that broad projection with `useBoxSize()` without an alias. F6 directly removed `useDraggable()`, element listeners, and terminal-wide raw mouse, then applied the same lifetime mechanism to the Fullscreen hooks in [targeted-pointer.md](./targeted-pointer.md). F2 proves only the shared attachment mechanism, not the later public semantics.
...
| Outside a render tree                                                   | `useBoxSize()` throws because there is no current Runtime app; a non-Box or foreign-app target also throws. Other terminal-bound composables retain their own explicit context behavior. |
...
A clean packed consumer using Vue 3.4.38, TypeScript 6, `skipLibCheck: false`, SFC template refs, and TSX refs originally proved both adapters without exposing `RenderedTargetController`, `useRenderedTargetRegistration`, or `TuiNode`. The same historical consumer reproduced vuejs/core#12639: the author ref remained non-null after detach while measurement reset and mouse ownership released, then recovered after insertion. Path 1's public type checks now additionally restrict `useBoxSize()` to `InstanceType<typeof Box>` and reject Text or arbitrary component refs.

The historical `target-lifetime` real-PTY journey keeps one component ref stable while its inner host moves through absent → `7x2` target → keyed `11x1` target at a new origin → removed during an active drag. It proves that SGR drag reporting changes from none → drag → none, the old cell does not start a drag, the new cell has exactly one live registration, measurement follows replacement and resets on final removal, active capture ends on removal, the alternate screen restores, the app exits zero, and terminal attributes before and after are identical. The visual-controller artifacts are intentionally ignored local review evidence; the fixture and PTY assertions are durable evidence for the retained lifetime mechanism, not for one public measurement shape.
...
No generic public `useRenderedTarget()` or target-ref type is justified yet. F4 through F6 now consume the internal lifetime rule and publish only the semantic authoring types their own journeys require.
