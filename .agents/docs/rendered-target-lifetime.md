...
The Vite HMR journey that separately covered a template-only rerender preserving the component instance, plus a script edit that reloaded it, was removed on 2026-07-25. Its Runtime-owned invariants are covered deterministically without a dev server in [`use-focus.test.tsx`](../../packages/runtime-tests/integration/composables/use-focus.test.tsx) — target loss does not queue acquisition or restore focus, a boundary that becomes hidden during suspension clears focus, and child visibility inside a true Fragment boundary is not reinterpreted — and in the Box-metrics tests for measurement following replacement. What only the HMR journey added was that Vue preserves component identity across a hot replacement, which is Vue's own behavior rather than a vue-tui contract.
