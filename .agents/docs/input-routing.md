...
Runtime closure Stage 2 proves this host contract through the exact issue #266 process shapes rather than a modeled stream alone. A committed packed fixture runs ignored child stdin and an A-to-B pipe inherited by a second child, each with input-free and active-input applications and three repetitions. Input-free children emit one final document and exit. Active semantic routing reports `stdin-not-tty` through the application result while emitting no child output, Vue warning, raw/listener/ref transition, cursor change, alternate-screen byte, paste or Kitty negotiation, mouse mode, or synchronized-output mode. The implementation registers `useInput` disposal before its immediate watcher can activate, routes only a preflight-known unavailable-input error through this silent expected-exit path, and keeps later acquisition failures on hosts that initially appeared controllable in the ordinary application error path. Fullscreen acquisition is now lazy: capability preflight happens before alternate-screen or cursor ownership, while a successful input demand acquires the Fullscreen surface before raw, paste, Kitty, mouse, or synchronized-output modes; an input-free Fullscreen mount acquires it at the first accepted commit. Runtime tarball `7a7c38f64fff43223d5fbadf662504482f8bd2268026c3c103c185acad166456` passes all 12 packed process repetitions.
...
The source audit, issue [#250](https://github.com/vuejs-ai/vue-tui/issues/250), issue [#266](https://github.com/vuejs-ai/vue-tui/issues/266), the first-party coding-agent and ScrollBox examples, the two-mode workbench and nested-PTY journeys, and the reverified peer routes supplied the evidence for this contract. They did not determine one complete attachment API without also deciding F4's logical focus and target ownership. The maintainer accepted the five recommended choices together on 2026-07-13 without adding a VOUCHED stamp; the alternatives remain below only as decision history.
...
This fail-fast live behavior follows Ink's useful guard/failure split while replacing its raw-mode fact with the semantic capability vue-tui actually supports. Applications can replace expected-host `try/catch` with the explicit preflight; a genuine later acquisition failure remains an error.
