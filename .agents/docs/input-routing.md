...
> **Status:** unstamped active F3 implementation record. F3.1 owns one serialized ingress and structural framing per physical stdin. F3.2 normalizes each structural event once into one shared immutable semantic fact. F3.3 captures non-reusable app-route leases when each framed fact begins and now routes parser-distinct facts independently of Node chunk grouping. F3.4 has an executable private policy model for explicit route order, independent continuation and default control, modal isolation, and normalized-sequence external fallthrough; it is not wired into live dispatch and does not prove byte-exact PTY transport. Public names and current-API disposition remain active. This record does not select a public event type, hook name, handler return convention, focus route, external-PTY API, or the final disposition of `useInput`, `usePaste`, `useMouseInput`, `useStdin`, and `exitOnCtrlC`.
