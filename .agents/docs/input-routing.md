...
Every active public global registration acquires its own controller input-demand lease. A selected private generation can independently request demand or remain logical-only: F4 needs to publish one complete fact-start focus topology even when no effective focus node can consume input, while an independent application-global owner may still drive facts through that topology. A demanded selection replacement acquires its lease, revalidates every proposed recipient after the host callback, publishes only if no later successful selection or clear won re-entrantly, and then releases the old generation. A logical-only replacement publishes before releasing the old demanded generation and never touches the host. Failed demand acquisition preserves the previous logical selection. Ending one owner releases its logical lease while another global or demanded selected generation keeps the shared physical resources continuously owned. An inactive public registration, an unselected private recipient, and a logical-only selected generation own no physical input. Routing reuses the existing transactional raw/listener controller rather than duplicating terminal mechanics.
