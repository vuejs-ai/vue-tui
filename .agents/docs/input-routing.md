...
> **Status:** historical unstamped F3 routing record. The parser, serialized ingress, terminal-resource ownership, and Runtime Ctrl+C default remain implementation evidence. The minimum public contract now exposes only `useInput()` key/text/paste facts, `isActive`, and the exact `{ preventDefault: true }` Runtime-default control; availability, focus/scopes, public propagation, external forwarding, and parser metadata are withdrawn. The direct `useStdin().stdin` decision remains vouched. See the [active ledger](./runtime-public-foundation-reaudit.md#exhaustive-retained-public-ledger).
