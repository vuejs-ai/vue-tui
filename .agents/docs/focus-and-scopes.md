...
The pre-F4 `useFocus()` registered `{ id, isActive }` during setup, appended the registration to one application array, and independently acquired raw input. `useFocusManager()` exposed the same array through string lookup, global enable/disable, registration-order movement, and `activeId`. It had no rendered target, nested scope, trap, restoration, or local-input attachment.
...
- inactive scopes remember their latest valid descendant. Reactivating a region restores that descendant, then an explicit autofocus target, without flattening sibling scopes; if scope activation commits before the preferred target's `v-if` host attaches, the preference remains pending and selects that target on the later authoritative rendered-tree commit, while a later explicit focus selection cancels the pending preference; an application-side region switch made while a modal owns the effective route is retained and becomes the restoration branch after the modal closes;
...
These rules are implemented in the private [focus policy](../../packages/runtime/src/focus/focus-policy.ts) and its [32 API-neutral journeys](../../packages/runtime/src/focus/focus-policy.test.ts). The model covers rendered reorder, explicit root initialization, hidden/disabled rejection, temporary restoration and explicit cancellation, prior-order removal fallback across detach-then-dispose batches, non-transfer to replacement identity, target-specific blur, one-shot autofocus, atomic retarget, independent and reactivated region memory, delayed host attachment after trapped-scope reactivation, later explicit-selection cancellation, nested and sibling modal restoration, empty-parent and atomic-trap-reactivation restoration, stale restoration targets, boundary/ancestor non-duplication, read-only and imperative-operation batch publication, targetless modal handling, finder/editor separation, identical mode-independent policy traces, and fact-start opening and closing boundaries. The pure model alone proves only semantic policy; F4 closure separately verified real Inline and Fullscreen hosts, Vue rendered ancestry, input-resource acquisition, package types, and terminal cleanup through the public implementation.
...
The packed first-party coding-agent journey exercises the targetless interval directly. Approval state activates the trapped scope before Vue necessarily attaches the approval Box ref, so Enter and Escape are approval-boundary actions in `useFocusScopeInput`, not target actions in `useFocusedInput`. The boundary consumes them, blocks composer input, and remains the correct owner after the target attaches. Reopening the `v-if` modal then proves the delayed target preference and outer composer restoration in both Inline and Fullscreen.

Handlers attached to the same target or scope form one all-run node layer and merge results monotonically, so composition does not turn registration time into hidden priority. `routing:"stop"` prevents later route nodes, not peers already captured at the same node. F3's fact-start snapshot keeps the selected route stable: if a handler opens or closes a modal or removes its target, the new, restored, or replacement selected target waits for the next framed fact. Independently captured application-global handlers for the same fact remain unaffected by selected-path staleness.

Rendered-target post-flush watchers may request a host-only reconciliation after the renderer commit has already accepted the same semantic tree. That reconciliation reuses the accepted focus generation only when no logical mutation is pending and the route boundary, owner, ordered ancestors, and external owner are identity-equal, while both the complete rendered-target sequence and the eligible sequential-target sequence are identity- and order-equal. Any logical mutation or change to those route or traversal inputs publishes a replacement generation. Exact no-op reuse preserves the current input-demand lease; it does not release and reacquire raw input.
...
The focus controller reconciles rendered facts after each authoritative renderer commit, but it does not publish a replacement generation merely because reconciliation ran. Only a logical mutation or a real change to the route, rendered-target order, or sequential-target order publishes a replacement. F3 still captures one selected generation at fact start and resolves it once; if that selected path becomes genuinely stale, it fails closed rather than resolving against the replacement. Independent application-global handlers captured for the same fact still run under their own F3 layer. Reading the route or any public readonly ref has no side effects and cannot reorder a pending scope activation. Target and scope disposal invalidates their route before Vue cleanup can expose a later fact to a stale recipient.
...
The app-owned controller is owned by every live or deterministic app plus every string-render document. Live and deterministic hosts join it to the real F3 runtime and pass it to F2 as the transaction owner; string rendering uses the same service in inert mode. The root package exposes the accepted `useFocus`, `useFocusScope`, `useFocusedInput`, `useFocusScopeInput`, `useExternalInput`, and `useFocusManager` adapters over that one controller. The old flat registry, string lookup, global enable/disable, automatic Escape blur, setup-order Tab default, per-focus raw acquisition, and subscriber-map test seam are removed without aliases. Public scopes use Vue injection for descendants and synchronously end outliving explicitly assigned targets' option watchers and F2 registrations when the scope ends. A controller with no target or scope lifetime owns no F3 selection, so applications without focus and private F3 fixtures are not displaced; inert string services validate handles but never attach hosts, select routes, acquire input, or call handlers.

The original 32 policy cases remain the policy-level evidence. Final controller and integration evidence additionally covers semantic no-op reconciliation preserving a split fact and demand lease; rendered and sequential reorder; visibility change; relative movement of a programmatic-only owner; scope deactivation; and a reactivated approval boundary receiving a lone legacy Escape in Inline and Fullscreen. Packed coding-agent, finder, independent-region, key/text/paste, duplicate-host recovery, invalid reactive target/scope recovery, recursive disposal, route order, HMR, clean Vue 3.4/TypeScript 6 consumption, real PTY, visual interaction, exact terminal cleanup, and final repository evidence are centralized in [Final closure evidence](./runtime-foundation-closure.md#final-closure-evidence) rather than restated as stale totals here.
...
The private policy experiment is deliberately not the production focus controller. It proved the semantic choices without exposing API names. F4 then completed the following dependency order:

All eight steps are implemented and verified:
...
This decision removed the public-shape stop boundary. F4's public runtime, repository migration, package consumption, real-host evidence, full gates, and independent review now pass, so F4 is Done. No VOUCHED stamp is implied by acceptance, implementation, or the private experiment.
