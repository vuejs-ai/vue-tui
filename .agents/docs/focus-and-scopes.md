...
The app-owned controller is owned by every live or deterministic app plus every string-render document. Live and deterministic hosts join it to the real F3 runtime and pass it to F2 as the transaction owner; string rendering uses the same service in inert mode. The root package exposes the accepted `useFocus`, `useFocusScope`, `useFocusedInput`, `useFocusScopeInput`, `useExternalInput`, and `useFocusManager` adapters over that one controller. The old flat registry, string lookup, global enable/disable, automatic Escape blur, setup-order Tab default, per-focus raw acquisition, and subscriber-map test seam are removed without aliases. Public scopes use Vue injection for descendants and synchronously end outliving explicitly assigned targets' option watchers and F2 registrations when the scope ends. A controller with no target or scope lifetime owns no F3 selection, so applications without focus and private F3 fixtures are not displaced; inert string services validate handles but never attach hosts, select routes, acquire input, or call handlers. The 32 policy cases, 26 controller cases, six-file public focus suite with 31 tests, packed coding-agent, finder, independent-region, focused key/text/paste, duplicate-host recovery, invalid reactive target/scope recovery, nested recursive disposal, Inline/Fullscreen traversal, route order, fact snapshots, keyed replacement, delayed modal-host attachment, no-focus behavior, non-TTY failure, string, screen-reader/final-output, suspension, HMR, clean Vue 3.4/TypeScript 6 consumption, real-PTY modal isolation/removal/restoration, visual interaction, exact terminal cleanup, full repository and fresh-CI gates, and independent review cover the completed implementation.
