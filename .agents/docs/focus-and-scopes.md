...
> **Status:** accepted, unstamped F4 target contract with the clean-slate public runtime implemented. The maintainer accepted all recommendations on 2026-07-13. Opaque ref-bound targets, scopes, focused and scope input, normalized external fallthrough, exact manager handles, renderer ownership, and direct removal of the prior registry/default/raw path now agree in source, package consumption, HMR, host-matrix, both-mode PTY/visual, full repository, fresh-CI, and independent-review evidence. Public finder and independent-region journeys plus the remaining boundary and richer terminal closure remain active; no VOUCHED stamp is implied.
...
The app-owned controller is owned by every live or deterministic app plus every string-render document. Live and deterministic hosts join it to the real F3 runtime and pass it to F2 as the transaction owner; string rendering uses the same service in inert mode. The root package now exposes the accepted `useFocus`, `useFocusScope`, `useFocusedInput`, `useFocusScopeInput`, `useExternalInput`, and `useFocusManager` adapters over that one controller. The old flat registry, string lookup, global enable/disable, automatic Escape blur, setup-order Tab default, per-focus raw acquisition, and subscriber-map test seam are removed without aliases. Public scopes use Vue injection for descendants and synchronously end outliving explicitly assigned targets' option watchers and F2 registrations when the scope ends. A controller with no target or scope lifetime owns no F3 selection, so applications without focus and private F3 fixtures are not displaced; inert string services validate handles but never attach hosts, select routes, acquire input, or call handlers. The 30 policy journeys, 22 controller journeys, public Inline/Fullscreen traversal, scopes, route order, fact snapshots, keyed replacement, disposal, invalid values, non-TTY failure, string hosts, screen-reader/final-output hosts, suspension, HMR, a clean Vue 3.4/TypeScript 6 consumer, the migrated coding-agent example, basic both-mode PTY and visual interaction, terminal restoration, full repository gates, fresh CI, and independent review cover the implemented path. Public finder and independent-region journeys, remaining duplicate/reactive/nested edges, and a richer modal/removal/restoration terminal journey remain closure work.
...
Steps 1–5 are implemented. The coding-agent part of step 6, most host and package cases in step 7, and the initial both-mode PTY and visual route in step 8 are complete. The remaining bullets name the active closure work.
