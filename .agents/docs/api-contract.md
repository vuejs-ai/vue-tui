...
F6 applies the same process to targeted mouse input. `@vue-tui/runtime/fullscreen`, the first supported public subpath, exposes only `useMouseEvent()` and `useMouseDrag()` plus the selected named types; it references rather than re-exports root `CellPoint` and `ElementTarget`, and does not duplicate `createApp()`, common components, focus, geometry, or render-session facts. `@vue-tui/testing` now exposes a parsed-physical-mouse driver and modeled reporting state so public tests exercise production click and drag state machines without hand-authoring SGR bytes. Package exports, declarations, exact value/type guards, template/TSX/JavaScript rejection, and clean Vue 3.4/TypeScript 6 consumption agree with the implemented surfaces. F6 remains Active only because its selected closure gate still requires an unlocked GUI-terminal observation of native selection and wheel capture/restoration; the exact runtime and testing export sets, behavior, direct v1 removals, and current evidence are recorded in [targeted-pointer.md](./targeted-pointer.md).
