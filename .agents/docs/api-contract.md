...
The independent higher layer now has two supported entries. The `@vue-tui/use` root exports exactly the `useInputWhileMounted`, `useKeyInput`, and `useTextInput` values plus the named `InputWhileMountedTargetRef` type. `useKeyInput()` and `useTextInput()` filter public `useInput()` delivery to the exact `"key"` and `"text"` event members respectively, pass the original frozen event, resolve a live handler ref only after a matching event arrives, and forward their options object unchanged. `useKeyInput()` excludes text even when it contains reliable nested `key` information, while `useTextInput()` preserves that optional information. Their event-member aliases stay private, so the package adds no named event types. `@vue-tui/use/components` exports exactly the `UseInputWhileMounted` value and generic named `UseInputWhileMountedProps` type. Both `useInputWhileMounted` authoring forms optionally select one public `TuiInputEvent` discriminator and narrow their handler or emitted event to that exact member; omission preserves the complete event union. The hook selector is static, while the component prop is reactive and retains template event inference. Their lifecycle contract is recorded in [input-while-mounted](./input-while-mounted.md); every `@vue-tui/use` behavior composes only the public Runtime root and does not expand Runtime's own foundation.
