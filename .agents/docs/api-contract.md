...
F5 applies the process to element geometry. `useElementGeometry()` follows a normal Vue component ref through rendered-host replacement and publishes one readonly ref whose frozen value always comes from one accepted paint generation. The guarded public types are `ElementTarget`, `CellPoint`, `CellRect`, `ElementGeometryFragment`, `ElementGeometry`, and `UseElementGeometryReturn`. Resolved states expose complete parent and dynamic-render-surface bounds plus exact local/parent/surface/visible-surface fragments; `unavailable`, `detached`, `pending`, `hidden`, `zero-size`, `fully-clipped`, and `visible` remain distinct. `useBoxMetrics()`, `measureElement()`, `BoxMetrics`, and `UseBoxMetricsReturn` were removed directly. Runtime, template, TSX, JavaScript, HMR, ScrollBox, package-output, and clean Vue 3.4/TypeScript 6 runtime/testing/components consumer gates cover the geometry cutover; caret ownership remains the active part of F5.
