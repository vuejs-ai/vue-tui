...
F6 applied the same process to targeted mouse input. `@vue-tui/runtime/fullscreen`, the first supported public subpath, exposes only `useMouseEvent()` and `useMouseDrag()` plus the selected named types; it references rather than re-exports root `CellPoint` and `ElementTarget`, and does not duplicate `createApp()`, common components, focus, geometry, or render-session facts. `@vue-tui/testing` exposes a parsed-physical-mouse driver and modeled reporting state so public tests exercise production click and drag state machines without hand-authoring SGR bytes. Package exports, declarations, exact value/type guards, template/TSX/JavaScript rejection, clean Vue 3.4/TypeScript 6 consumption, and the direct native macOS Terminal selection/wheel observation agree with the implemented surfaces, so F6 is complete. The exact runtime and testing export sets, behavior, direct v1 removals, and closure evidence are recorded in [targeted-pointer.md](./targeted-pointer.md).
