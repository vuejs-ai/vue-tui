...
The independent higher layer now has two supported entries. The `@vue-tui/use` root exports exactly the `useInputWhileMounted` and `useTextInput` values plus the named `InputWhileMountedTargetRef` type. `useTextInput()` filters public `useInput()` delivery to the exact `"text"` event member, passes the original frozen event with any optional reliable nested `key`, resolves a live handler ref only after a matching event arrives, and forwards its options object unchanged. Its text-member alias stays private, so the package adds no named event type. `@vue-tui/use/components` exports exactly the `UseInputWhileMounted` value and generic named `UseInputWhileMountedProps` type. Both `useInputWhileMounted` authoring forms optionally select one public `TuiInputEvent` discriminator and narrow their handler or emitted event to that exact member; omission preserves the complete event union. The hook selector is static, while the component prop is reactive and retains template event inference. Their lifecycle contract is recorded in [input-while-mounted](./input-while-mounted.md); every `@vue-tui/use` behavior composes only the public Runtime root and does not expand Runtime's own foundation.
