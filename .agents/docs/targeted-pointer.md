...
F6 established exactly two mouse value exports on `/fullscreen`, `useMouseEvent` and `useMouseDrag`. Completed F8 deliberately adds the third current value export, `useTextSelection`, plus its selection types; the pointer surface itself remains the two F6 values and named mouse types recorded here. Those mouse types are `MouseButton`, `MouseHandlerResult`, `CellDelta`, `MouseModifiers`, `TuiMouseClickEvent`, `TuiMouseWheelEvent`, `TuiMouseEventMap`, `MouseEventHandler`, `TuiMouseDragEvent`, `MouseDragHandler`, `UseMouseEventOptions`, `UseMouseDragOptions`, and `UseMouseDragReturn`. The subpath references the root `CellPoint` and `ElementTarget` types and Vue's ref utility types without re-exporting them. It does not re-export root application, component, focus, geometry, or render-session values.
...
| Any requested mode resolving to a screen-reader transcript, final stream, or other non-targetable output presentation | Remain inert and quiet; the mouse primitive owns this absence behavior and does not require the application to inspect a general session object first.           |
