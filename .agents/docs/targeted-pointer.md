...
All automated, package, PTY, and headless visual gates above currently pass locally. The remaining closure observation is deliberately narrow: run the journey in an unlocked GUI terminal, confirm native drag-selection and terminal wheel behavior are unavailable during active reporting, remove the final target, and confirm both return. The current macOS session reports the console locked, `loginwindow` in front, and the display inactive and asleep; a screen capture fails with `could not create image from display` even though screen-recording and accessibility permissions are present. No visible terminal pixels are therefore available to observe honestly, so F6 must remain Active rather than treating PTY state as GUI evidence.

Once the desktop is unlocked and awake, build the runtime and components, then run `node --import=tsx packages/runtime-tests/integration/pty/fixtures/fullscreen-origin.tsx 18 targeted-mouse` in an actual GUI Terminal window. With `targets=hidden`, confirm ordinary drag-selection and terminal wheel behavior work; press `a` and confirm selection no longer produces native highlight while wheel input reaches the application; press `x` and confirm both native behaviors return; press `q` and confirm them once more in the restored shell. This observation must be recorded directly rather than inferred from mouse control sequences.
