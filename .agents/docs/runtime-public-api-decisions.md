...
- **Ruling:** `RenderToStringOptions` has exactly `readonly width?: number` and `readonly height?: number`. Width defaults to 80 and must be a positive bounded integer. Height defaults to 24 and must be either a positive bounded integer or positive `Infinity`. A finite height is the maximum available root layout height: shorter ordinary output is not padded, while ordinary dynamic layout does not paint beyond the bound. `height: Infinity` selects the former vertically unbounded document behavior. Components observe the same values through `useLayoutSize()`.
...
- **Limits:** This does not add an asynchronous string renderer, root-props overload, VNode or app overload, string-specific styling dialect, color option, terminal mode, streams, lifecycle barriers, or live lifecycle. `width` and `height` replace the earlier experimental `columns`-only option without aliases. Complete-document callers must opt into `height: Infinity`; preserving an implicit unbounded default is not allowed to complicate the primary layout API.
