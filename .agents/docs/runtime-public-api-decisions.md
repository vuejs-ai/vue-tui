...
- **Ruling:** The mounted document host acquires no managed terminal-input or screen resources. `useInput()` registrations are accepted for shared-component setup but remain inert. `useStdin()` still returns the exact mounted `Readable` for caller-owned direct observation, but reports no raw-mode support and Runtime never changes that stream's raw state, even when the supplied stdin is independently a TTY. Inline and Fullscreen mode requests have the same result there. Runtime still owns its mounted Vue lifecycle, borrowed streams, output barriers, failures, and cleanup; `renderToString()` remains the synchronous API that directly returns one initial document without those mounted resources.
- **Limits:** Both document paths are supported contracts rather than undefined behavior, but they remain secondary: failures inside their stated behavior are bugs, while their narrower needs do not require parity with live input, resize, alternate-screen, cursor, pointer, or other interactive Runtime behavior and must not complicate the primary live-TTY API. Mounted non-TTY has no width or height options; callers needing a deterministic non-default document size use `renderToString({ width, height })`. A custom Writable that truthfully exposes live TTY capabilities follows the primary live-TTY contract.
...
- **Source:** Yunfei, 2026-07-24, final current layout-host review after run-verifying pinned Ink 7.0.4 and current stable 7.1.1 non-TTY behavior and distinguishing synchronous `renderToString()` from its mounted document counterpart; no durable session URL is available, so this entry is the durable record.
...
- **Ruling:** Before user setup or terminal mutation, Runtime resolves and validates options, defaults, stdout host class, mode, stdout and stderr protocol state, stdout ownership, and live-TTY Fullscreen positive-dimension requirements. It then reserves stdout, creates rollback ownership, installs stream observers, and installs accepted console protection before user component setup. If setup creates active managed input demand on a live-TTY host, Runtime validates stdin before acquiring raw mode or other terminal resources; the mounted document host keeps that demand inert. Only after those checks succeed may it enter alternate screen, raw mode, mouse modes, or paint. Every acquired resource immediately registers inverse cleanup, and failure rolls back in reverse order. A later inactive-to-active input transition rechecks stdin capability where managed input is supported.
...
- **Limits:** Focus, scopes, modal routing, priority, and propagation remain application or higher-layer policy composed through activation. `useInput()` does not promise relative handler ordering as a routing mechanism. Live-TTY Inline and Fullscreen use the same event contract. An active live subscription requires controllable mounted stdin and fails explicitly when that capability is unavailable; inactive subscriptions create no input demand. The mounted non-TTY document host and `renderToString()` permit the composable for shared setup but never invoke its handler. A future phase-aware or raw-input facility, if justified, must be a separate opt-in primitive rather than changing this default delivery contract.
...
- **Ruling:** Raw-only use does not start Runtime's normalized parser, Kitty negotiation, or bracketed-paste reporting. The caller owns direct stream listeners and their cleanup. Direct stream observation and `useInput()` may see the same physical input and have no safe ordering, deduplication, protocol-filtering, or byte-exact composition guarantee. A non-TTY `Readable` remains observable while reporting no controllable raw mode. In the mounted non-TTY document host, the hook returns the exact mounted `Readable` for direct observation but reports no raw-mode support and never changes raw state, even if that stream independently exposes TTY controls. In `renderToString()`, the hook instead receives an isolated inert `Readable`, reports no raw-mode support, never touches `process.stdin`, and produces no input.
