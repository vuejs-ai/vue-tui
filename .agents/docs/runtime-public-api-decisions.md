...
- **Limits:** This decides the default non-TTY output policy, not the still-Open exact ordering of several simultaneously eligible `Static` blocks. It does not add a public cadence override. A custom Writable that happens to emulate TTY capabilities is governed by the capabilities it truthfully exposes.
...
- **Limits:** This does not add special `v-if` or `v-show` detection, promise reversible visibility, or copy Ink's `items`, render-function, and style API. Exact cross-instance ordering, hidden-ancestor behavior, placement, nesting, and Static-specific failure timing remain Open. Inline non-TTY append behavior and `renderToString()` behavior are decided elsewhere in this ledger. Private host nodes and prepare, accept, abandon, backpressure, and rollback transactions remain implementation mechanisms.
- **Why:** Only Runtime can coordinate irreversible terminal history with the replaceable frame. Vue already owns collection rendering, instance identity, conditional creation, and component cleanup, so Runtime needs only the one-block primitive. Pinned Ink v7.0.4 runs confirm the useful behavioral baseline: empty `items` may later produce output, unmount does not erase committed output, and remount creates a fresh producer that repeats the block. vue-tui keeps those simple semantics while expressing collection composition through Vue rather than importing React's collection component shape. The measured peer relationship is recorded in [vue-tui ↔ Ink](./ink-divergences.md).
...
- **Why:** The public machud monitor uses the operation for real `--once` and `--snapshot` output that can be piped, used in CI, or inspected without a TTY, while its tests use the same path for deterministic component documents. Only Runtime can reproduce terminal cell measurement, wrapping, Yoga layout, Text styling, and Static extraction. The contract follows the run-verified Ink 7.0.4 synchronous baseline while expressing Static through Vue tree semantics. machud also proves the shared-component case: the snapshot tree obtains `exit` for an input handler that can never run in the inert string host, so permitting the hook without inventing an exceptional lifecycle is the smaller behavior.
- **Source:** Yunfei, 2026-07-23, current Runtime public API review after the production-use audit, peer comparison, explicit review of the complete behavior batch, and correction of the unsupported `exit()` exception policy; no durable session URL is available, so this entry is the durable record.
