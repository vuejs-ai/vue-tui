...
### `Box` covers Ink's style grammar; `Text` keeps its nine fields

[VOUCHED @hyfdev 2026-07-25]

- **Ruling:** `BoxProps` covers every Ink Box style except `display`, which Vue's `v-show` replaces: 62 fields including `alignContent`, `aspectRatio`, per-edge `borderColor`, `borderDimColor` and `borderBackgroundColor`, and a `borderStyle` that accepts all eight `cli-boxes` frames or a complete custom frame object. `TextProps` keeps its nine fields, which already matched Ink exactly. The exact declarations live in the [public API contract](./api-contract.md#box-and-text).
- **Limits:** A custom frame must supply all eight corner and edge characters; an unknown frame name is still rejected. This does not add `zIndex`, layers, opacity, markup, hyperlinks, a broad `style` object, or the `aria-*` props — the first six are absent from Ink too, and the last went with the screen-reader experiment.
- **Why:** Yunfei reopened the earlier "coherent minimum" after seeing that it described a public surface rather than a capability. The renderer already implemented every one of these behaviors with Ink-parity comments; only the prop declaration and its closed validator excluded them. The narrowing therefore cost authors real expressiveness — a two-tone frame or a focus-highlighted top edge required nested Boxes — while buying no simplicity in the implementation.
- **Source:** Yunfei, 2026-07-25, after a field-by-field comparison against the pinned Ink v7.0.4 baseline established that `TextProps` was never reduced and that the missing Box fields were public-surface-only omissions.
