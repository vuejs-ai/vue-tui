...
Judgments Yunfei actually expressed about the target public surface of `@vue-tui/runtime` — selections, acceptances, and rejections. A finished implementation, a passed review, resemblance to a peer, or silence is not acceptance. Never invent a rationale. Entries record the judgment, not the full API contract; the implemented shape lives in the [current branch API contract](./api-contract.md), with executable evidence in code and tests. Edit entries in place; git keeps history.
...
The branch's exact export inventory is guarded in code and tests. Entries below record only Yunfei's expressed judgments; evidence-determined technical conclusions belong in the API contract rather than this ledger.
...
- **Limits:** This rejects an edge-only interpretation of minimum API; it does not automatically accept every Ink alias or every convenience unrelated to an already accepted primitive. The exact `BoxProps` fields were settled separately on 2026-07-25 by [`Box` covers Ink's style grammar](#box-covers-inks-style-grammar-text-keeps-its-nine-fields), which this ruling constrains rather than decides: the question there was which fields exist at all, while this entry only forbids deleting a shorthand that already earns its place.
...
- **Source:** Yunfei accepted the Box surface on 2026-07-25, then on 2026-08-02 explicitly allowed adding the Runtime capability if peer evidence supported it while reviewing Table's multiline alignment. The earlier section stamp was removed because changing the nine-field ruling changes the words it covered.

### Box measurement is one complete parent-relative layout rectangle

[VOUCHED @hyfdev 2026-07-25]

- **Ruling:** Replace the experimental `useBoxSize()` with the exact `useBoxMetrics()` public declaration and behavior in the [exact `useBoxMetrics` contract](./api-contract.md#useboxmetrics). It accepts a readonly current-app direct-Box ref and returns the named `UseBoxMetricsReturn`, whose readonly numeric refs are `width`, `height`, parent-relative `left`, and parent-relative `top`, plus readonly boolean `hasMeasured`. The four numeric values are zero before a measurement is available; `hasMeasured` distinguishes that state from a legitimately zero-sized measured Box.
...
- **Source:** Yunfei, 2026-07-24, acceptance of the complete [exact `useBoxMetrics` contract](./api-contract.md#useboxmetrics) after pinned-peer comparison and bounded adversarial capability review, followed by explicit rejection of the redundant imperative API; no durable session URL is available, so the linked contract section is the pinned artifact.
...
- **Ruling:** Adopt the exact public declarations, component-root normalization, unavailable behavior, host semantics, and lifecycle contract in the [exact `useFocus` contract](./api-contract.md#usefocus). In summary, Runtime exports `FocusTarget` as a readonly Vue ref to a current-app `ComponentPublicInstance | null | undefined`, exports one `UseFocusReturn` with a readonly boolean `Ref` plus void `focus()` and `blur()` operations, and provides `useFocus()` and `useFocus(target)` as two explicit overloads with the same return type. It has no options object or distinct targeted return.
