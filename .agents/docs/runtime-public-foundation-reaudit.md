...
Final local evidence on 2026-07-24 satisfies the implemented behavior gate. `vp run ready` passed formatting, all nine package and example builds, warning-free lint, package and PTY-fixture type checks, clean packed consumers on Vue 3.4.38 and 3.5.34 with TypeScript 6.0.3, and all Runtime, Vite, Testing, Components, integration, PTY, and example suites. The clean consumer resolves the packed `@vue-tui/runtime/package.json`, reads the shipped version-matched visual guide, checks the current root and subpath exports, and rejects `/internal` and `/fullscreen`; the tarball contains no private `internal` entry file. Those tooling-subpath expectations are evidence of the current implementation and must change with the selected internal-package boundary. Application-facing higher layers and examples have no Runtime-internal imports, while repository-only white-box tests use an unpacked private build entry where exact internal symbol identity is required.

The final post-review run covered 737 Runtime tests, 75 Testing tests, 34 Components tests, 933 Runtime integration tests, 120 real-PTY tests, 28 Vite tests, and six example tests. The clean consumer additionally exercises accepted-Static cleanup and subsequent keyed reorder/removal on both supported Vue lines, rejects every hidden or superseded mount string before side effects, and proves that the current testing bridge does not expose repository-only mount controls through a caller-owned `mount` wrapper. Two independent bounded reviewers challenged the earlier candidate: the public-boundary review corrected decision-ledger provenance, records-map routing, superseded inventory, and operating-status text; the Static-cleanup review reproduced half-patched anchors, ancestor-removal and immediate-teardown settlement loss, Vue-handled hook timing, and delayed first-cause inversion. The final closure review additionally found and closed the hidden testing-bridge channel, stale Vite fixture options, partial console-patch installation leak, and missing Vite-worker console constructor compatibility. Its final pass found no remaining issue in those code boundaries and identified one stale completion sentence in this record, corrected above.
