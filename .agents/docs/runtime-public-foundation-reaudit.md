...
The 2026-07-24 bounded review supplied technical evidence for the final surface. Real Machud and ScrollBox call sites justify the three narrow layout facts; the separately published Vite and Testing packages justify the two integration subpaths; the shipped-guide lookup justifies package metadata; and the public-only compositions above distinguish higher-layer behavior from future additive Runtime interaction features. Yunfei then accepted current rendered-tree ordering and reduced Static eligibility to mounted identity, explicitly rejecting visibility-dependent policy and declining to make unsupported placement, nesting, diagnostics, recovery, or Static-specific failure timing part of the public contract. With that final choice implemented and locally verified, the minimum Runtime ownership and API foundation is complete; future work is additive application behavior, optional public-only higher layers, components, or a separately justified Runtime primitive.
