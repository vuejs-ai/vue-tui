...
PR #265 is open at remote head `3d7e197`; the local worktree is 23 commits ahead at `dd8596f`. The PR conversation contains one automated overview of its first three files, no inline review threads, and no human package-boundary review. The existing test volume therefore proves behavior of the current experiment, not correctness of its public boundary.
...
All rendering hosts support Box and Text. `renderToString()` accepts only an options object with an optional positive safe-integer `columns` value, default 80; unknown keys and attempts to pass live mode, rows, or presentation controls fail before rendering. It has no terminal resources, input, focus, caret, or asynchronous flush, and always releases its Vue and Yoga tree before returning or propagating a render failure. A visual non-TTY mount emits a monotonic stream document rather than dynamic cursor rewrites. Screen-reader presentation uses the semantic linearizer rather than the visual cell painter. Invalid props and render failures remain ordinary synchronous Vue/Runtime errors with complete cleanup.

### Box and Text prop-family audit

The current `BoxProps` contains 67 real props plus the four `onClick`, `onMouseDown`, `onMouseUp`, and `onWheel` rejection tombstones; `TextProps` contains 11 real props plus the same tombstones. Retaining the component values does not retain that whole vocabulary. The target keeps 29 Box props and eight Text props, with no compatibility aliases for rejected experimental props.

The shared target domains are exact:

- A cell count is a safe integer greater than or equal to zero. A signed cell offset or margin is any safe integer. A flex factor is any finite number greater than or equal to zero.
- A percentage width is an ordinary non-negative decimal matching `^(?:0|[1-9]\d*)(?:\.\d+)?%$`; percentages above 100 are permitted, while signs, whitespace, exponents, bare numeric strings, and truncated decimals are not.
- The exported `Color` type is one of the 16 canonical ANSI palette spellings (`black`, `red`, `green`, `yellow`, `blue`, `magenta`, `cyan`, `white`, `gray`, `redBright`, `greenBright`, `yellowBright`, `blueBright`, `magentaBright`, `cyanBright`, or `whiteBright`) or an exact six-digit `#RRGGBB` value. `grey` and `blackBright` are not separate aliases. Unknown names, empty strings, short hex, `rgb(...)`, and `ansi256(...)` are not public values. Runtime may downgrade a valid RGB color to the terminal's supported palette without changing the input contract. The named type lets public component and theme authors describe values accepted by Runtime without duplicating or extracting its grammar.
...
| `flexGrow`                                       | Retain a non-negative finite factor; default/reset `0`.                                                                                                                         | Machud and ScrollBox need Runtime to distribute remaining sibling space.                                                                                                                                           |
| `flexShrink`                                     | Retain a non-negative finite factor; default/reset `1`.                                                                                                                         | ScrollBox and ErrorOverview need Runtime's sibling shrink calculation.                                                                                                                                             |
...
| `marginLeft`                                     | Retain a signed cell count; default/reset `0`.                                                                                                                                  | The v-show layout journey uses it and it cannot be derived from another accepted prop.                                                                                                                             |
...
For visual candidates, Runtime-validated wrong JavaScript types fail with `TypeError`, out-of-range numbers fail with `RangeError`, and unsupported strings fail with an error naming the prop and value before any host patch. Vue-shaped Boolean props retain Vue's normal bare-attribute and coercion behavior rather than adding a second Runtime Boolean protocol; the vouched truthy literal `aria-hidden="false"` case is the deliberate edge. Omission, `undefined`, or reactive prop removal restores the declared default rather than leaving stale Yoga or painter state. An invalid reactive replacement, layout failure, or paint failure does not publish a partial layout, element size, focus topology, text mapping, caret, or pointer generation; the last accepted facts remain authoritative until the application enters its fatal lifecycle. `display="none"` is absence, not a visible zero-sized node.
...
The retained four primitives already have representative examples and broad render tests, but their public types must be rerun through the clean-consumer matrix after the other removals. The component package and docs must not import the removed convenience values from Runtime. No renderer rewrite is proposed: the current Box/Text/Yoga/paint implementation is reusable evidence and remains unless focused migration tests reveal a correctness issue.
...
The host matrix is explicit: visual Inline commits history; Fullscreen requested but screen-reader-fallback effective Inline commits a transcript; a visual non-TTY stream commits each settled Static prefix in document order while retaining only the mutable tail for teardown; string rendering prepends each Static block to the dynamic document; effective visual Fullscreen rejects component presence before emitting history or a new frame. A stream `write()` returning `false` still accepts the bytes exactly once and settles the instance while Runtime waits for drain. A throwing write has indeterminate physical handoff and seals the instance as abandoned without retry.

Exploratory host probes already confirmed tree-order initial siblings, append, keyed removal and reorder, remount replay, gate-busy latest-tree selection, `write(false)` exactly-once handoff, visual Inline, screen-reader fallback, current final and forced-live non-TTY implementations, string rendering, and the template-versus-script HMR boundary. The forced-live probe is historical evidence for the transaction only, not a retained host policy. Before implementation is accepted, those probes must become focused target-policy tests, the indeterminate-write reproduction must fail then pass with per-host sealing, the coding-agent must use stable message keys, and the packed and visual Inline-history journeys must pass. Until then, the current collection contract remains experimental evidence rather than an accepted target.
