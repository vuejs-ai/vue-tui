...
This is the implementation and technical evidence record for the public boundary currently present on PR #265. On 2026-07-22 Yunfei reopened that surface for item-by-item review. The [Runtime public API decision ledger](./runtime-public-api-decisions.md) remains authoritative for his expressed judgments; this re-audit records unstamped implementation results and evidence, not acceptance. Its exact live `Static` ordering, hidden-ancestor behavior, placement, nesting, and Static-specific failure timing remain Open in that ledger. No vouch stamp was added or moved. The PR remains unmerged and unreleased.
...
The final post-review run covered 737 Runtime tests, 75 Testing tests, 34 Components tests, 933 Runtime integration tests, 120 real-PTY tests, 28 Vite tests, and six example tests. The clean consumer additionally exercises accepted-Static cleanup and subsequent keyed reorder/removal on both supported Vue lines, rejects every hidden or superseded mount string before side effects, and proves that the public testing bridge does not expose repository-only mount controls through a caller-owned `mount` wrapper. Two independent bounded reviewers challenged the earlier candidate: the public-boundary review corrected decision-ledger provenance, records-map routing, superseded inventory, and operating-status text; the Static-cleanup review reproduced half-patched anchors, ancestor-removal and immediate-teardown settlement loss, Vue-handled hook timing, and delayed first-cause inversion. The final closure review additionally found and closed the hidden testing-bridge channel, stale Vite fixture options, partial console-patch installation leak, and missing Vite-worker console constructor compatibility. Its final pass found no remaining issue in those code boundaries and identified one stale completion sentence in this record, corrected above.
...
The 2026-07-24 bounded review supplied technical evidence for the remaining candidate surface without adding a vouch stamp. Real Machud and ScrollBox call sites justify the three narrow layout facts; the separately published Vite and Testing packages justify the two integration subpaths; the shipped-guide lookup justifies package metadata; and the public-only compositions above distinguish higher-layer behavior from future additive Runtime interaction features. Live Static ordering, hidden-ancestor behavior, placement, nesting, and Static-specific failure timing now have explicit implementation and test evidence, but that evidence does not replace Yunfei's review of the Open contract. Until he accepts or revises it, this document supports a nearly complete implemented candidate rather than a completed minimum Runtime foundation.
