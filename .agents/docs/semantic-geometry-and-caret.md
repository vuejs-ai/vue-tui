...
| [`useDraggable()`](../../packages/runtime/src/composables/useDraggable.ts) | F2-bound Fullscreen behavior whose `x/y` are caller state plus pointer displacement.                                                                          | Its values are not element geometry. F6 has selected direct replacement with a rendered-host-owned drag lifecycle; implementation is pending.                                                 |
| [`MouseTarget.rect`](../../packages/runtime/src/mouse/events.ts)           | The latest clipped Fullscreen hit rectangle; event offsets are rebased from that visible origin.                                                              | It is not the full element rectangle, exists only through pointer delivery, and its shared runtime object is mutable through JavaScript or `any`. F6 has selected exact F5 fragments instead. |
