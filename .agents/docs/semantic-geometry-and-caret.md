...
> **Status:** unstamped completed F5.1 audit, F5.2 public proposal, F5.3 private paint-derived geometry service, and public geometry cutover. The current source, repository consumers, host behavior, executable failures, pinned peers, exact Vue signatures, and nested-editor policy prototype were reverified on 2026-07-13. Paint produces frozen atomic geometry generations with exact complete-grapheme ownership, wrapped fragments, sparse caret slots, F2 lifetime, both-mode surface coordinates, clear/suspend invalidation, resize recovery, and honest unavailable states. Geometry tracing is demand-driven: each frame snapshots live targets and the ancestor paths needed to reach them, so an unobserved subtree keeps the ordinary paint cost and a root with no geometry consumers performs no provenance work. `useElementGeometry()` now publishes a runtime-readonly projection without private caret slots, ScrollBox consumes full resolved geometry, and the former scalar/Yoga measurement APIs are removed. Caret arbitration, terminal transport, and final acceptance evidence remain active.
...
| [`ScrollBox`](../../packages/components/src/scroll-box/scroll-box.vue)     | Uses two public `useElementGeometry()` projections and caches each last resolved full parent height.                                                          | It proves semantic geometry has a real common-component consumer; F7 still owns movement results and boundary routing.                                                                  |
...
No low-level output-origin escape hatch survived the proposal: repository and external search found no non-editor consumer, while retaining it would preserve a second owner model whose cleanup can invalidate the semantic arbiter. The public geometry cutover directly removed `useBoxMetrics()`, `BoxMetrics`, `UseBoxMetricsReturn`, and `measureElement()`, and migrated `ScrollBox`. The next caret unit directly removes `useCursor()` and `CursorPosition`. No alias, warning period, or compatibility branch is added.
...
Visual live and at-teardown streams may still expose deterministic geometry while mounted; targeted caret transport remains unavailable on every non-TTY output. A modeled visual TTY exposes the matching production geometry and caret state. Screen-reader and string presentation expose `unavailable`, including before a target attaches; when a visual surface becomes available again, an attached target returns to `pending` and an unattached target returns to `detached`. Calling the geometry composable outside a render tree also returns a stable `unavailable` projection rather than throwing. Static content never becomes a durable target. Suspension publishes `unavailable`, continuation repaints a new generation before geometry or caret becomes visible again.
...
A wrapped nested Text can start after a prefix, fill the first line, and continue at column zero. A rectangle rooted at the first fragment misses the continuation; its bounding rectangle includes unrelated cells. Surface-only fragments also cannot translate an element-local point because each wrapped row can start at a different surface column. `fragments` is therefore an array of exact local/parent/surface/visible-surface mappings even though boxes normally contain one entry. This is the same richer paint geometry F6 will later consume without changing F5's public target or publishing pointer behavior early.

## Executable proposal prototype

[`semantic-geometry-caret-prototype.test.ts`](../../packages/runtime/src/geometry/semantic-geometry-caret-prototype.test.ts) is an API-shaped private policy prototype, not the production controller. Eighteen tests demonstrate that the selected types and rules can represent:
...
The prototype settled representational sufficiency and removed the public-shape ambiguity. The private production service derives geometry from authoritative paint and joins F2 lifetime. Its focused implementation tests cover Box and nested Text mappings, exact Unicode ownership, explicit and soft wrapping, clipping, authored hidden and zero-content guards, Static and Transform boundaries, truncation, resize, failed-paint discard, retargeting, frozen generations, Inline and Fullscreen clear/repaint, screen-reader fallback, live-surface suspension/continuation, and final-output snapshot preservation when only input resources suspend. The public and package geometry gates now pass. F4 caret ownership, writer transport, testing cursor visibility, and final real-terminal caret journeys remain.
...
3. **Next:** build one per-app caret registry joined to F4 target identity and the geometry generation. Resolve one focused owner after paint and feed its surface point or `undefined` into the existing mode writer; directly remove targetless `useCursor()`.
