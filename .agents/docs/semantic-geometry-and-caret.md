...
> **Status:** unstamped completed F5.1 audit, F5.2 public proposal, and F5.3 private paint-derived geometry service. The current source, repository consumers, host behavior, executable failures, pinned peers, exact Vue signatures, and nested-editor policy prototype were reverified on 2026-07-13. Paint now produces frozen atomic geometry generations with exact complete-grapheme ownership, wrapped fragments, sparse caret slots, F2 lifetime, both-mode surface coordinates, clear/suspend invalidation, resize recovery, and honest unavailable states. Geometry tracing is demand-driven: each frame snapshots live targets and the ancestor paths needed to reach them, so an unobserved subtree keeps the ordinary paint cost and a root with no geometry consumers performs no provenance work. Public cutover, ScrollBox migration, caret arbitration, terminal transport, and final acceptance evidence remain active.
