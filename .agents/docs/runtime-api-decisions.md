...
- **Limits:** This is an output-capability control, not a theme, palette, concrete foreground color, mutable session control, or reason to expose Runtime's internal Chalk instance. Capability remains fixed and privately owned by each render session after resolution. Environment variables are automatic CLI-user policy only; ordinary tests use `color` rather than mutating process color state, while resolver and real-terminal tests may deliberately verify automatic detection.
