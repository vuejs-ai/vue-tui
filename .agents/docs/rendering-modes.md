...
below its content: Runtime supplies `flexShrink: 0` to in-flow children of vertical stacks during
layout unless the application authors a value, compensating for Yoga's missing content-based
automatic minimum height.
...
- [`render-session.test.ts`](../../packages/runtime/tests/session/render-session.test.ts) pins pure surface resolution, including identical non-TTY results for both requested modes.
