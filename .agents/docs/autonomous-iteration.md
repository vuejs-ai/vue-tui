...
Close F6, Fullscreen targeted pointer, the sole [current checkpoint](./api-foundation-roadmap.md#current-checkpoint). F1 supplies truthful rendering-mode and host facts, F2 supplies rendered-target lifetime, F3 supplies normalized input and routing, F4 supplies logical focus and scopes, and completed F5 supplies paint-derived semantic geometry. Begin with the existing listener props, `useMouseInput()`, `useDraggable()`, mouse controller, real application journeys, and pinned peers; audit their ownership and failure modes before selecting the public hook name, signature, propagation, coordinates, reporting demand, `/fullscreen` path, or current-API dispositions. Do not publish F7 or F8 behavior early.
...
- Keep later foundations free of new public APIs while F6 is active. A useful internal prototype may supply evidence, but it does not authorize publishing F7–F8 early, selecting F6 names before its audit, or turning F2's private controller, F3's private topology, or F5's private insertion slots into generic public APIs.
