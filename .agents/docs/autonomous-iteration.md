...
Advance F3, normalized input and routing, the sole [current checkpoint](./api-foundation-roadmap.md#current-checkpoint). F3.1 has one serialized structural ingress per physical stdin, F3.2 has one shared immutable semantic fact per structural event, and F3.3 has non-reusable app-route leases captured at physical event start. The current unit must now test priority, continuation, default prevention, and external-owner fallthrough through representative journeys. Do not publish a new event API while materially different routing semantics remain untested.

1. script a coding-agent composer/approval journey and one finder or terminal-workbench journey that require global interrupt, local editing, paste boundaries, modal ownership, unhandled-key continuation, and external PTY fallthrough without manual `isActive` booleans;
2. compare pinned Ink and the already-recorded peers only for the concrete delivery questions exposed by those journeys; state candidate handled, continue, component-default prevention, and external-owner behavior in plain observable terms before choosing names or types;
3. prototype explicit internal layers for application-global commands, the active boundary, the later focused owner and logical ancestors, delayed defaults, and an optional external owner while keeping continuation and default prevention independent;
4. prove a handled fact stops where intended, an explicit continuation reaches the next semantic owner, preventing a default does not imply route termination, and a fully continued event reaches an explicit external owner exactly once with an honest recoverable representation;
5. give current `useInput`, `usePaste`, and direct stdin access an explicit retain, replace, or remove disposition only after the experiment shows one coherent model; then cover template/TSX types, both modes, non-TTY and string hosts where callable, teardown, HMR, and real PTY input/fallthrough;
6. run focused gates, full `vp run ready`, fresh `CI=true vp run ci`, clean package consumers, and independent review; update the F3 contract, commit coherent review boundaries, and open draft stacked PRs without adding a VOUCHED stamp.
...
- PR [#254](https://github.com/vuejs-ai/vue-tui/pull/254) merged as `f1ce02b`; normal visual full-screen already owns the fixed surface recorded in [fullscreen-output.md](./fullscreen-output.md). F3 must treat that renderer behavior, F1's public session facts, F2's [rendered-target lifetime](./rendered-target-lifetime.md), F3.1's serialized ingress, F3.2's semantic fact boundary, and F3.3's route-lifetime control as established inputs.
