...
Advance F5, semantic geometry and caret, the sole [current checkpoint](./api-foundation-roadmap.md#current-checkpoint). F1 supplies truthful rendering-mode and host facts, F2 supplies rendered-target lifetime, F3 supplies normalized input and routing, and completed F4 supplies one public logical-focus owner with scopes, restoration, and target-bound delivery. Begin with an audit of current geometry, measurement, cursor, drag, focus-handle, and pointer-target surfaces plus representative editor/caret journeys and pinned peer mechanisms. Research and executable evidence must identify the shared semantic element and coordinate requirements before any public F5 API shape is selected.

Operational checkpoint: PR [#265](https://github.com/vuejs-ai/vue-tui/pull/265) remains the single open foundation PR, based directly on merged `main` `25df36d`. Commit `f7a2ff9` closes F4's public finder, coding-agent, independent-region, normalized-paste, duplicate-host, invalid-reactive-transition, and nested-scope scenarios. Commit `55710ae` adds the richer Inline/Fullscreen PTY and visual-controller journey with modal isolation, focused-host removal, restoration, identical routing, and exact terminal cleanup. After `78a55c3` exposed that a staged simulated Kitty reply could still miss the runtime's 200ms window on a contended Linux runner, `d88622f` removed automatic capability detection from this unrelated F4 journey, retained exact explicit Kitty PUSH/POP cleanup assertions, and left auto-detection to its dedicated PTY suite. The current records mark F4 Done and F5 Active. Reverify the remote head and gates before every new unit rather than treating `d88622f` as permanent branch state.
...
11. **Current F5 research unit:** audit current and consumer geometry/caret behavior, define representative editor and clipped/hidden/removed/retargeted host journeys, and reverify relevant peer mechanisms before proposing a semantic element, rectangle, or caret API. Do not publish F5 names or pull F6 pointer, F7 scrolling, F8 selection, a component catalog, or any VOUCHED stamp forward from research alone.
