...
Operational checkpoint: PR [#265](https://github.com/vuejs-ai/vue-tui/pull/265) remains the single open foundation PR, based directly on merged `main` `25df36d`. F5.3 starts from green remote head `afff161`, keeps the selected public signatures unexported, and adds the private production authority plus two renderer correctness prerequisites: spatially stable wide-glyph left clipping and complete overflow containment before and after Transform. Reverify the remote head and gates before every new unit rather than treating this starting head as permanent branch state.
