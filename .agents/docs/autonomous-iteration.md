...
Advance F4, logical focus and focus scopes, the sole [current checkpoint](./api-foundation-roadmap.md#current-checkpoint). F1 supplies truthful rendering-mode and host facts, F2 supplies rendered-target lifetime, and completed F3 supplies normalized immutable input, explicit route outcomes, independent application-global handlers, delayed defaults, external fallthrough semantics, and transactional input ownership. The maintainer accepted every recommendation in [logical focus and focus scopes](./focus-and-scopes.md) on 2026-07-13 without adding a VOUCHED stamp. F4.1 research, public shapes, clean-slate dispositions, host behavior, implementation gates, the 30-journey policy, the 22-journey app-owned controller, and renderer/context ownership are complete. The live work is the accepted public composable cutover; ordinary implementation choices no longer require a maintainer stop.

Operational checkpoint: PR [#265](https://github.com/vuejs-ai/vue-tui/pull/265) remains the single open foundation PR, based directly on merged `main` `25df36d`. Live and deterministic apps now own one injected app-level controller joined to F2 and F3, while string rendering owns the same service in inert mode. With no F4 lifetime the service leaves F3 topology unselected, preserving applications without focus and isolated F3 fixtures. The next unit directly replaces the old public focus registry, defaults, and raw path with the accepted composables. Reverify the remote head and gates before every new unit rather than treating a recorded hash as permanent.
...
2. **Completed F4.1 audit:** current focus is a setup-scoped flat string-ID registry with stale hidden targets, setup-order traversal, disabled programmatic focus, duplicate-ID ambiguity, no restoration, per-target raw demand, and no F3 focused route. Issue #250, the coding-agent example, ScrollBox, pinned `mo`, `machud`, and Herdr establish the journey requirements.
...
9. **Current implementation unit:** directly replace the flat string-ID controller, per-focus raw acquisition, automatic Escape blur, and live-state framework Tab with the accepted target/scope/input/external composables and initial integration evidence. Do not pull F5 geometry, F6 pointer, a focus component catalog, or any VOUCHED stamp forward.

F1 through F3 are Done. The public readonly render session supplies truthful host facts, the private rendered-target controller supplies one proven attachment lifetime without exposing renderer nodes, and normalized input supplies one event and routing model. F4 may consume all three but must not reopen the two screen models, treat the Inline default as product hierarchy, or publish geometry, pointer, scroll-composition, or selection contracts early.
...
- Keep later foundations free of new public APIs while F4 is active. A useful internal prototype may supply evidence, but it does not authorize publishing F5–F8 early or turning F2's private controller or F3's private topology into generic public APIs.
