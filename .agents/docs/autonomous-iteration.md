...
9. **Active proposal:** give explicit retain, replace, or remove dispositions for `useInput`, `usePaste`, and `exitOnCtrlC`; select normalized public event and handler contracts without pulling F4 focus or F6 pointer work forward; then implement the uniquely supported or maintainer-accepted shape and rerun the full gates before closing F3.
