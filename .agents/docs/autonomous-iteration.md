...
Advance F4, logical focus and focus scopes, the sole [current checkpoint](./api-foundation-roadmap.md#current-checkpoint). F1 supplies truthful rendering-mode and host facts, F2 supplies rendered-target lifetime, and completed F3 supplies normalized immutable input, explicit route outcomes, independent application-global handlers, delayed defaults, external fallthrough semantics, and transactional input ownership. The maintainer accepted every recommendation in [logical focus and focus scopes](./focus-and-scopes.md) on 2026-07-13 without adding a VOUCHED stamp. F4.1 research, public shapes, clean-slate dispositions, host behavior, implementation gates, and the 30-journey private policy are complete. The F3 logical-only topology option, F2 owner transaction, and focus-policy checkpoint now provide the three rollback and publication seams required by the app-owned controller. The live work is public integration in the dependency order recorded there; ordinary implementation choices no longer require a maintainer stop.

Operational checkpoint: PR [#265](https://github.com/vuejs-ai/vue-tui/pull/265) remains the single open foundation PR, based directly on merged `main` `25df36d`. Commits `b4ded26`, `23edfd1`, and `342bb9a` separate logical topology from physical input demand, batch complete F2 target ownership changes, invalidate removed subtrees before cleanup callbacks, and checkpoint the last accepted focus generation for transactional rollback. The next coherent unit implements the app-owned controller over those seams. Reverify the remote head and gates before every new unit rather than treating a recorded hash as permanent.
...
7. **Current implementation unit:** replace the flat controller with the application-owned focus/scope service, derive immutable target/scope/Tab/external route generations over F3, and bind host order, inherited hidden state, atomic retarget, and synchronous subtree invalidation to F2. Publish the accepted composables only after their runtime behavior exists. Do not pull F5 geometry, F6 pointer, a focus component catalog, or any VOUCHED stamp forward.
