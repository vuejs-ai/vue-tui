...
Advance F5, semantic geometry and caret, the sole [current checkpoint](./api-foundation-roadmap.md#current-checkpoint). F1 supplies truthful rendering-mode and host facts, F2 supplies rendered-target lifetime, F3 supplies normalized input and routing, and completed F4 supplies one public logical-focus owner with scopes, restoration, and target-bound delivery. The [F5.1 audit and F5.2 public proposal](./semantic-geometry-and-caret.md) now establish the shared mechanism, exact signatures, state taxonomy, fragments, current-API removals, and acceptance journeys. The next unit builds one private paint-derived geometry service with F2 lifetime and atomic generations; do not publish the proposed surface until that authority passes its internal host, clipping, nested-Text, retarget, and resize gates.

Operational checkpoint: PR [#265](https://github.com/vuejs-ai/vue-tui/pull/265) remains the single open foundation PR, based directly on merged `main` `25df36d`. This proposal run started from clean green remote head `88c280d`, where F5.1 already fixed forced-live non-TTY caret leakage, recorded the global-owner cleanup gap, and completed the source/host/peer audit. F5.2 adds only the API-shaped private policy prototype and records the exact target contract; no current public export or production geometry/caret behavior changes in this unit. Reverify the remote head and gates before every new unit rather than treating this starting head as permanent branch state.
...
12. **Completed F5.2 public proposal:** direct Vue targets beat a second generic handle; one frozen discriminated snapshot exposes parent/surface bounds and exact local/parent/surface/visible fragment mappings; caret authors supply an element-local rendered cell plus an exact F4 focus handle; one owner binds to one focus handle; private row/slot mappings represent empty and trailing insertion cells. Marker, logical-offset, arbitrary-active, single-rectangle, and retained targetless-cursor alternatives are rejected. The sixteen-test private prototype proves representational, relationship, reactive-recovery, focus-disposal, validation, and owner-policy sufficiency without claiming renderer integration.
13. **Current F5 geometry implementation unit:** build one private paint-derived service with F2 attachment, synchronous detachment, post-paint atomic publication, exact box and wrapped nested-Text fragment mappings, private caret rows/slots, clipping, hidden/zero/pending states, both-mode surface coordinates, resize, retarget, host unavailability, and rollback-safe cleanup. Keep the public proposal and all F6–F8 behavior unexported until the internal authority passes.
