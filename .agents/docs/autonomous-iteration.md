...
Advance F3, normalized input and routing, the sole [current checkpoint](./api-foundation-roadmap.md#current-checkpoint). F3.1 has one serialized structural ingress per physical stdin. F3.2 has one shared immutable semantic fact per structural event and cached adapters for the current hooks. The current unit must add per-route event-start lifetime before testing priority, continuation, default prevention, and external-owner fallthrough. Do not publish a new event API while materially different routing semantics remain untested.

1. add an internal typed route registry in which every hook attachment receives a non-reusable lease, while a handler ref update keeps the same lease and reads the latest callback;
2. capture each app's route snapshot when a physical event begins, inherit it across split CSI, paste, pointer, and UTF-8 input, and filter live leases once when delivery begins so current broadcast order and re-entrant EventEmitter behavior remain the control;
3. prove component A cannot hand a split event to replacement B, while unrelated persistent routes, other apps, bootstrap input during Kitty detection, suspension, HMR, and mouse-versus-key classification retain their correct event-start behavior without changing raw or terminal-mode ownership;
4. script a coding-agent composer/approval journey and one finder or terminal-workbench journey that require global interrupt, local editing, paste boundaries, modal ownership, unhandled-key continuation, and external PTY fallthrough without manual `isActive` booleans;
5. compare pinned Ink and the already-recorded peers only for the concrete delivery questions exposed by those journeys; state candidate handled, continue, component-default prevention, and external-owner behavior in plain observable terms before choosing names or types;
...
- PR [#254](https://github.com/vuejs-ai/vue-tui/pull/254) merged as `f1ce02b`; normal visual full-screen already owns the fixed surface recorded in [fullscreen-output.md](./fullscreen-output.md). F3 must treat that renderer behavior, F1's public session facts, F2's [rendered-target lifetime](./rendered-target-lifetime.md), F3.1's serialized ingress, and F3.2's semantic fact boundary as established inputs.
