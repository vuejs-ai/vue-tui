...
Advance F5, semantic geometry and caret, the sole [current checkpoint](./api-foundation-roadmap.md#current-checkpoint). F1 supplies truthful rendering-mode and host facts, F2 supplies rendered-target lifetime, F3 supplies normalized input and routing, and completed F4 supplies one public logical-focus owner with scopes, restoration, and target-bound delivery. The [F5.1 audit, F5.2 public proposal, F5.3 private geometry service, and public geometry cutover](./semantic-geometry-and-caret.md) now establish the shared mechanism, exact signatures, state taxonomy, complete-grapheme paint authority, fragments, sparse slots, lifecycle behavior, current-API removals, and acceptance journeys. The next unit implements the focus-bound caret registry, resolves one eligible owner after paint, and directly replaces `useCursor()`; do not publish F6–F8 behavior early.

Operational checkpoint: PR [#265](https://github.com/vuejs-ai/vue-tui/pull/265) remains the single open foundation PR, based directly on merged `main` `25df36d`. The branch now includes the F5.3 private authority, demand-scoped geometry tracing, and the public geometry cutover. `useElementGeometry()` is the supported paint-derived observation API, ScrollBox consumes it, and the old Yoga measurement surface is gone. Reverify the remote head and gates before every new unit rather than treating any checkpoint hash as permanent branch state.
...
12. **Completed F5.2 public proposal:** direct Vue targets beat a second generic handle; one frozen discriminated snapshot exposes parent/surface bounds and exact local/parent/surface/visible-surface fragment mappings; caret authors supply an element-local rendered cell plus an exact F4 focus handle; one owner binds to one focus handle; private sparse slots represent empty and trailing insertion cells. Marker, logical-offset, arbitrary-active, single-rectangle, and retained targetless-cursor alternatives are rejected. The eighteen-test private prototype proves representational, relationship, reactive-recovery, focus-disposal, validation, and owner-policy sufficiency without claiming renderer integration.
...
15. **Current F5 caret registry:** join one per-app caret registry to F4 focus identity and accepted paint geometry, choose exactly one eligible owner after paint, feed its surface cell into the existing mode writer, publish `useCaret()`, and directly remove targetless `useCursor()`. Keep F6–F8 behavior queued until caret ownership and transport pass their focused gates.

F1 through F4 are Done. The public readonly render session supplies truthful host facts, the private rendered-target controller supplies one proven attachment lifetime without exposing renderer nodes, normalized input supplies one event and routing model, and public logical focus supplies one mode-independent owner and scope path. F5 may consume all four but must not reopen the two screen models, treat the Inline default as product hierarchy, collapse focus into caret or selection state, or publish pointer, scroll-composition, or selection contracts early.
...
- Keep later foundations free of new public APIs while F5 is active. A useful internal prototype may supply evidence, but it does not authorize publishing F6–F8 early, selecting F5 names before its audit, or turning F2's private controller or F3's private topology into generic public APIs.
