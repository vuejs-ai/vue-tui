...
8. **Maintainer decision:** choose whether managed semantic routing excludes non-TTY streams, and whether `setRawMode`, `isRawModeSupported`, and the mount `rawMode` option remain public or give way to route-owned demand; then finish demand-driven Kitty, remaining error, template/TSX type, package-consumer, and restoration gates;
