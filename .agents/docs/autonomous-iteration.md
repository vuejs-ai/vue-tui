...
Advance F5, semantic geometry and caret, the sole [current checkpoint](./api-foundation-roadmap.md#current-checkpoint). F1 supplies truthful rendering-mode and host facts, F2 supplies rendered-target lifetime, F3 supplies normalized input and routing, and completed F4 supplies one public logical-focus owner with scopes, restoration, and target-bound delivery. The [F5.1 audit](./semantic-geometry-and-caret.md) now establishes the shared internal mechanism and acceptance journeys. The next unit must write exact competing Vue-facing signatures, prototype the nested focused-editor journey across both modes, and select a public contract only when author responsibility, ownership, clipping, Unicode, host availability, and arbitration agree.

Operational checkpoint: PR [#265](https://github.com/vuejs-ai/vue-tui/pull/265) remains the single open foundation PR, based directly on merged `main` `25df36d`. This run started from green remote head `37664d2`, which records F4 Done and F5 Active after the completed public and both-mode terminal journeys. F5.1 then found and corrected one independent output-safety defect: an active targetless cursor declaration could leak movement and visibility controls through either forced-live non-TTY writer. It also records the unresolved multi-owner cleanup behavior as an expected-failure contract and completes the audit without changing the public geometry or caret surface. Reverify the remote head and gates before every new unit rather than treating this starting head as permanent branch state.
...
12. **Current F5 public-proposal unit:** write exact direct-ref versus derived-handle, atomic geometry, local-cell versus marker/text-position, and focus-eligibility signatures; prototype the nested focused editor against them; then select or return the remaining honest alternatives. Do not pull F6 pointer, F7 scrolling, F8 selection, a component catalog, or any VOUCHED stamp forward.
