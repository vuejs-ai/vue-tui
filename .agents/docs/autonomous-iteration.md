...
Advance F4, logical focus and focus scopes, the sole [current checkpoint](./api-foundation-roadmap.md#current-checkpoint). F1 supplies truthful rendering-mode and host facts, F2 supplies rendered-target lifetime, and completed F3 supplies normalized immutable input, explicit route outcomes, independent application-global handlers, delayed defaults, external fallthrough semantics, and transactional input ownership. F4 must use those mechanisms to make one effective logical focus owner follow rendered lifetime, traversal, disabled and hidden state, nested scopes, modal traps, and restoration without asking applications to connect `isFocused` to `useInput({ isActive })`. The next unit is research and a complete proposal: audit the current flat string-ID registry and consumers, run the coding-agent approval, finder, independent-region, removal, and restoration journeys, recheck peer focus mechanisms, and give `useFocus` plus `useFocusManager` an explicit retain/replace/remove disposition. Do not publish a target/local/external hook, target type, or scope API before the maintainer accepts the complete contract.

Operational checkpoint: PR [#265](https://github.com/vuejs-ai/vue-tui/pull/265) remains the single open foundation PR, based directly on merged `main` `25df36d`. The current coherent unit closes F3's public implementation and activates F4. Reverify the remote head and gates before every new unit rather than treating a recorded hash as permanent.
...
2. **Next evidence unit:** audit current `useFocus`, `useFocusManager`, focus defaults, rendered-target seams, every first-party example, issue #250, and representative external consumers; record exactly which setup-scope and string-ID assumptions fail.
3. **Next journey unit:** make the coding-agent approval overlay, a finder/editor, two independent regions, disabled or hidden targets, target removal, modal close, and restoration executable under Inline and Fullscreen without selecting API names in the fixture.
4. **Next peer check:** reverify focus target identity, traversal order, nested scopes, traps, restoration, disabled/hidden behavior, and input attachment in the recorded peers; use primary sources and distinguish feasible mechanisms from vue-tui product choices.
5. **Next proposal:** present one complete F4 contract covering ownership, rendered lifetime, target identity, programmatic focus, traversal, scopes, traps, restoration, normalized local input, external-owner isolation, unavailable hosts, and exact dispositions for `useFocus` and `useFocusManager`.
6. **Stop boundary:** return the complete decision packet to the maintainer before public implementation. No VOUCHED stamp, target type, focus component catalog, F5 geometry, or F6 pointer work is implied.
