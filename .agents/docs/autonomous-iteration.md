...
Advance F3, normalized input and routing, the sole [current checkpoint](./api-foundation-roadmap.md#current-checkpoint). F3.1 has one serialized structural ingress per physical stdin, F3.2 has one shared immutable semantic fact per structural event, and F3.3 has non-reusable app-route leases captured per framed fact with parser-distinct route selection independent of Node chunk grouping. F3.4 defines the private candidate policy, and the live bridge captures atomic selected-topology generations plus recipient activations with each application snapshot, resolves them once, dispatches the policy in the real controller, and delays Ctrl+C, Escape and Tab defaults under both modes without publishing an API. Product-boundary, Herdr, implementation, and real nested-PTY evidence select semantic facts plus normalized UTF-8 rather than post-protocol raw spans. The maintainer-vouched contract retains `useStdin().stdin` as the actual mounted stream while excluding its raw bytes from framework event semantics and safe routing-composition guarantees. The accepted host boundary is implemented: non-TTY streams are excluded from managed routing; public raw-mode operations and mount policy are removed; semantic demand owns raw/listener/ref/Kitty state across modes, output cadence, HMR, suspension, failure, deterministic hosts, and real PTYs. On 2026-07-13 the maintainer accepted the complete unstamped public contract: normalized facts, a required synchronous route result, one all-run application-global hook, an independent availability query, fail-fast activation on an unavailable live host while inactive and string-host registration remain inert, removal of `usePaste`, public `Key`, and `exitOnCtrlC`, retention of raw stdin, and F4 ownership of target/local/external attachment. The next implementation unit publishes that contract, migrates repository consumers directly, and runs the F3 closure gates without pulling F4 or F6 forward.

Operational checkpoint: PR [#265](https://github.com/vuejs-ai/vue-tui/pull/265) remains the single open foundation PR, based directly on merged `main` `25df36d`. Commit `e7aeebe` closes the accepted input-host and lifecycle implementation; the current docs-only unit records the accepted public contract without implementing it. Reverify the remote head and gate before every new unit rather than treating either hash as permanent.
...
4. **Completed in the live controller:** extend F3.3 leases to an atomic selected boundary, supplied focus path, defaults and external owner; freeze recipients before callbacks; keep registration time out of priority; prove split and same-chunk replacement, re-entry, modal isolation, delayed defaults, external fallthrough and shared-stdin error isolation under Inline and Fullscreen without pulling F4 focus selection forward;
5. **Completed:** retain the narrower normalized UTF-8 external contract and prove slow protocol isolation plus ordered key, text, paste, uninterpreted and invalid-input delivery through an explicit adapter into a real child PTY in Inline and Fullscreen;
6. **Completed:** under the vouched direct-stdin contract, retain the physical `stdin` field as the actual mounted stream outside framework event and safe routing-composition guarantees; do not add a second brokered route for semantic fallthrough;
7. **Completed:** acquire input demand transactionally across both modes, screen-reader and final-stream output, shared stdin, suspension and continuation, teardown, replacement, independent globals, acquisition rollback, and the real nested-PTY journey;
8. **Completed:** exclude non-TTY managed routing; remove public raw-mode operations and mount policy; make semantic-route demand own raw/listener/ref and Kitty negotiation; and pass error, template/TSX/SFC/JavaScript, package-content, clean-consumer, HMR, deterministic-host, and real-PTY restoration evidence;
9. **Accepted:** the [public input contract](./input-routing.md#accepted-public-input-contract) gives exact dispositions, event and handler types, availability behavior, and the F3/F4 attachment split. Rejected alternatives remain as decision history; no VOUCHED stamp was added.
10. **Next unit:** implement the accepted F3 public contract, migrate every repository consumer without compatibility aliases, add public/type/package and live-host evidence, rerun the full gates, close F3 only when its completion criteria pass, then activate F4.
