...
Advance F5, semantic geometry and caret, the sole [current checkpoint](./api-foundation-roadmap.md#current-checkpoint). F1 supplies truthful rendering-mode and host facts, F2 supplies rendered-target lifetime, F3 supplies normalized input and routing, and completed F4 supplies one public logical-focus owner with scopes, restoration, and target-bound delivery. The [F5.1 audit, F5.2 public proposal, and F5.3 private geometry service](./semantic-geometry-and-caret.md) now establish the shared mechanism, exact signatures, state taxonomy, complete-grapheme paint authority, fragments, sparse slots, lifecycle behavior, current-API removals, and acceptance journeys. The next unit implements and exports `useElementGeometry()`, migrates ScrollBox, and directly removes `useBoxMetrics()` and `measureElement()`; do not begin caret arbitration or publish F6–F8 behavior early.

Operational checkpoint: PR [#265](https://github.com/vuejs-ai/vue-tui/pull/265) remains the single open foundation PR, based directly on merged `main` `25df36d`. F5.3 implementation commit `56aaa99` keeps the selected public signatures unexported and adds the private production authority plus two renderer correctness prerequisites: spatially stable wide-glyph left clipping and complete overflow containment before and after Transform. Its first remote gate exposed unconditional provenance work on an unobserved 256 KiB final-output Text; the correction scopes every paint frame to live targets and their ancestor paths before the public cutover begins. Reverify the remote head and gates before every new unit rather than treating any checkpoint hash as permanent branch state.
...
12. **Completed F5.2 public proposal:** direct Vue targets beat a second generic handle; one frozen discriminated snapshot exposes parent/surface bounds and exact local/parent/surface/visible fragment mappings; caret authors supply an element-local rendered cell plus an exact F4 focus handle; one owner binds to one focus handle; private sparse slots represent empty and trailing insertion cells. Marker, logical-offset, arbitrary-active, single-rectangle, and retained targetless-cursor alternatives are rejected. The eighteen-test private prototype proves representational, relationship, reactive-recovery, focus-disposal, validation, and owner-policy sufficiency without claiming renderer integration.
...
14. **Current F5 public geometry cutover:** implement and export `useElementGeometry()`, migrate ScrollBox to the atomic snapshot, and directly remove `useBoxMetrics()` and `measureElement()` from runtime code, declarations, tests, docs, examples, package guards, and clean consumers. Keep `useCaret()`, writer transport, and F6–F8 behavior queued until this public geometry layer passes.
