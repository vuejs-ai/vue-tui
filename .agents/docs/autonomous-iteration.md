...
7. **Completed for private selected topologies:** acquire input demand transactionally under `rawMode: "auto"`; cover both modes, screen-reader and final-stream output, shared stdin, suspension and continuation, teardown, replacement, independent globals, acquisition rollback, and the existing real nested-PTY journey;
8. **Active implementation:** enforce the accepted non-TTY exclusion and remove `setRawMode`, `isRawModeSupported`, and the mount `rawMode` option; make semantic-route demand own raw/listener/ref and Kitty negotiation, then finish remaining error, template/TSX type, package-consumer, and restoration gates;
9. run focused gates, full `vp run ready`, fresh `CI=true vp run ci`, clean package consumers, and independent review; update the F3 contract, commit coherent review boundaries, and push them to the single active foundation PR without adding a VOUCHED stamp.
