...
Operational checkpoint: PR [#265](https://github.com/vuejs-ai/vue-tui/pull/265) remains the single open foundation PR, based directly on merged `main` `25df36d`. Commit `f7a2ff9` closes F4's public finder, coding-agent, independent-region, normalized-paste, duplicate-host, invalid-reactive-transition, and nested-scope scenarios. Commit `55710ae` adds the richer Inline/Fullscreen PTY and visual-controller journey with modal isolation, focused-host removal, restoration, identical routing, and exact terminal cleanup; `78a55c3` makes its bounded Kitty reply deterministic under a contended CI worker. The current records mark F4 Done and F5 Active. Reverify the remote head and gates before every new unit rather than treating `78a55c3` as permanent branch state.
