...
6. **Blocked on maintainer confirmation:** remove the physical `stdin` field from the supported render-tree surface; do not add a second brokered route or re-expose a host-owned stream as a framework escape hatch;
7. after that decision, cover automatic input demand, template/TSX types, both modes, non-TTY and string hosts where callable, teardown, HMR, package consumption, restoration, and the remaining `useInput` and `usePaste` dispositions;
8. run focused gates, full `vp run ready`, fresh `CI=true vp run ci`, clean package consumers, and independent review; update the F3 contract, commit coherent review boundaries, and push them to the single active foundation PR without adding a VOUCHED stamp.
