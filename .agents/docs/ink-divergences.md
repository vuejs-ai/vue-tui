...
The current vue-tui implementation already has first-call exit and final `waitUntilExit()` settlement, but its pre-mount and teardown availability errors on `waitUntilRenderFlush()` do not yet implement the accepted target and remain tracked in [TODOs](./runtime-public-api-decisions.md).
...
The pinned upstream behavior was run-verified with real `PassThrough` streams and cross-checked against Ink's [non-TTY and explicit-override tests](https://github.com/vadimdemedes/ink/blob/40b3a7578811fd616341ca4e31cc7748aeeff12f/test/components.tsx#L1152-L1247), [alternate-screen tests](https://github.com/vadimdemedes/ink/blob/40b3a7578811fd616341ca4e31cc7748aeeff12f/test/components.tsx#L1712-L1757), and Static tests. The accepted target and implementation gaps are recorded in the [mount host review](./api-contract.md#exact-public-shapes).
...
- **vue-tui:** a composable's `setup()` runs **once**, so reactive state cannot be a plain
  ...
  updates and re-renders the template; read these as `.value`. Current stateful composables
  follow this model: `useFocus().isFocused` is a readonly ref, `useLayoutSize()` returns readonly
  `width` and `height` refs, and `useBoxMetrics()` returns readonly refs for `width`, `height`,
  parent-relative `left`, parent-relative `top`, and `hasMeasured`.
...
- **Current implementation:** the active-registration stack and pre-component installation preserve every Vue warning prefix, and an application's registration remains active through Vue cleanup before the previous registration or native console is restored. Focused nested-app, warning, cleanup-order, and throwing-cleanup tests guard the contract.
...
#### Historical: `<Transform>` screen-reader rider and all-comment slot collapse
...
- **Why:** a child set that renders nothing equals omitting the child — letting
  ...
  removed rather than transferred to this narrower visual statement. The private Transform
  mechanism and its wrapper regression tests were later removed entirely with the redundancy
  cleanup; the comparison above is retained only as historical evidence.
...
> **Unstamped lifetime note:** component-instance support cannot stop at reading `$el` when the ref changes. The public component instance can remain identical while its rendered root moves through `null`, insertion, keyed replacement, removal, or a template-only HMR rerender; Vue 3.4 can also leave a stale non-null ref to an already detached host. The renderer reconciles internal ref-bound registrations by resolved host identity after every commit and invalidates removed subtrees synchronously. Current `useBoxMetrics()` and `useFocus(target)` require that mechanism; the old pointer and caret adapters were removed. See [rendered-target-lifetime.md](./rendered-target-lifetime.md).
...
- **Why:** a self handle covers ordinary local programmatic focus without a global string namespace, while applications or a public-only higher layer can build distant lookup with a `Map<string, UseFocusReturn>`. Vue template refs additionally let Runtime solve the ancestor-`v-show` case that Ink's logical-only registration cannot see. Both overloads stay in one controller so targetless and targeted components cannot claim focus independently. The closed public names are `FocusTarget` and `UseFocusReturn`; the accepted behavior and evidence are in the [`useFocus` review](./api-contract.md#usefocus).
...
- **vue-tui:** public APIs are Vue **composables** (`useFocus`, `useInput`, ...). Where a
  ...
  `UseXReturn` convention (`UseAppReturn`, `UseStdinReturn`, `UseLayoutSizeReturn`,
  `UseBoxMetricsReturn`, `UseFocusReturn`); the remaining composables
  return `void` or small unexported inline shapes — never an `XProps` type.
  `XProps` is reserved for component props (`BoxProps`/`TextProps`, derived via
...
The current boundary and retained type-system evidence are in [accessibility-api](./removed-experiments.md#screen-reader-presentation-and-aria). Runtime no longer exposes ARIA props, named ARIA types, a screen-reader presentation, or a hidden transcript path; this entry describes the removed experiment.
...
- **Why:** the two modes differ in [history ownership](./intent.md#rendering-modes), and Ink's alternate screen does not make that distinction at all. Ink's `alternateScreen` keeps the relative inline writer, so its Static bytes go to the alt buffer exactly as they go to the main screen and the _terminal_ holds them; that is why it works there, and it is the same reason Ink's dynamic frame gets pushed down by any other write. vue-tui's Fullscreen owns its viewport and delegates nothing, so no terminal-owned history exists to hand content to. Keeping it would need one of two shapes, and both are worse than the alternative. Retaining the bytes but releasing the subtree — Static's defining behavior — paints rows that no longer have components behind them, so a future renderer-owned hit map would have holes whose existence depends on whether the app happened to use Static. Retaining the subtree instead makes Static something else: not write-once, nothing handed to the terminal, and unbounded growth that only the framework holds, so nobody can trim it. It would also mean one component name with two contracts chosen by ambient mode, which the authoring rules reject outright.
- **Alternative:** a Fullscreen app keeps history in its own state and renders it, usually through a bounded `ScrollBox`. That is not a workaround: the content stays live, so it lays out, clips, scrolls, and will hit-test like everything else, and the application decides what to trim. A future "commit once and skip re-rendering" optimization is a separate memo-style primitive that behaves identically in both modes, not a second meaning for `Static`.
...
- **vue-tui:** deterministic option, output-stream, ownership, and Fullscreen checks run before user setup or terminal mutation. Subsequent acquisition installs reverse cleanup as each resource is obtained. Once mounted, a failure of stdout or of an accepted stdout or stderr write, or an actual stdin `error` while managed input is active, records the first cause, tears down, and rejects `waitUntilExit()` after accepted output finishes or is abandoned. Normal stdin end or close merely stops future input, `EPIPE` is not silent success, lost output without an `Error` receives a stable Runtime error, and later cleanup failures do not replace an earlier cause.
- **Why:** a caller needs one truthful result for operations Runtime actually attempted. Leaked modes, process-level crashes, pending exit promises, and false clean completion all violate that result, but absence of future stdin bytes is not itself a failed operation. `useInput()` therefore subscribes to any mounted `Readable`, conditionally acquires an exposed raw-mode API, and rolls back only an actual acquisition or stream error.
...
### Measured Text uses one conservative whole-cell budget

Measured Text follows the repository-wide [one-pass layout rule](../../AGENTS.md): its ordinary layout completes in one layout-engine pass, without treating a backend change or a backend-discovered defect as justification for feedback layout.
...
- **vue-tui:** a finite allocation converts a positive fractional width to the complete terminal cells it contains (`max(1, floor(width))`; a non-positive allocation remains zero). Measured Text nodes share a private Yoga Config with point-grid projection disabled, so their computed layouts retain the resolved pre-grid width after the single layout call. Paint applies the same pure whole-cell conversion directly to that retained width and clamps it to the parent content box. Runtime therefore records no callback candidate, carries no paint override, never dirties Text after layout, and never invokes layout again. For 4.25 columns, both an origin-aligned parent that rounds to four cells and a parent whose fractional absolute origin makes it round to five cells wrap at the same conservative four-cell budget. The extra rounded parent cell in the latter case remains unused rather than changing measured height after layout.
- **Why:** Yoga 3.2.1 deliberately [rounds measured Text nodes outward](https://github.com/facebook/yoga/blob/v3.2.1/yoga/algorithm/PixelGrid.cpp#L95-L132), but its measure callback does not know the node's eventual absolute offset. That offset changes pixel-grid rounding: the same text, fractional constraint, and measure mode can finish with a four- or five-cell parent solely because its absolute origin differs. Making the rounded cells authoritative therefore requires feedback; preserving Text's already-resolved fractional geometry lets measurement and paint derive one budget instead. Cached layouts, paint-only prop changes, and measure-mode transitions cannot alter that derivation because callback history is not an input. Taffy Yoga exposed the callback-order dependency tracked in [#283](https://github.com/vuejs-ai/vue-tui/issues/283), but neither the defect nor this architecture rule is specific to selecting Taffy or any other backend.
- **Evidence:** [`text-measure-parity.test.tsx`](../../tests/runtime/integration/layout/text-measure-parity.test.tsx) pairs the same 4.25-column constraint at origins `0` and `0.25`, covers a fractional offset, an overflow-hidden parent, positive sub-cell width and height constraints, a paint-only reactive update, and a `5 → 4 → 5` resize that reuses Yoga's cached layout. [`single-pass-text-measurement.test.ts`](../../packages/runtime/tests/host/layout-guards/single-pass-text-measurement.test.ts) directly asserts one layout-engine call; [`text-measurement.test.ts`](../../packages/runtime/tests/host/yoga/text-measurement.test.ts) locks the pure whole-cell policy for undefined, zero, sub-cell, and fractional `Exactly` and `AtMost` requests.
- **Scope:** Text measurement does not drive feedback layout. [Zero-content containment](#degenerate-boxes-do-not-lay-out-or-paint-children-when-the-content-area-is-gone) may repeat layout because the resolved content rectangle is unavailable beforehand and hiding an in-flow child changes its ancestors' layout; each repeat hides at least one newly discovered child. [Inline](#inline-bounds-its-live-region-instead-of-deleting-terminal-history) and Document rendering first calculate natural height and repeat with the terminal bound only on overflow, because supplying that bound initially changes percentage-height descendants in short content; the Document branches are in [`render.ts`](../../packages/runtime/src/render.ts) and [`render-to-string.ts`](../../packages/runtime/src/render-to-string.ts). These existing bounded exceptions do not justify feedback from Text or a new multi-pass feature; extending or changing any of them requires explicit review against the one-pass rule.
...
- **Why:** Yoga layout, clipping, row replacement, and coordinated output must agree on one fixed origin. The run-verified Ink behavior above is the concrete failure this avoids: a dependency's `console.log` pushes the whole "fullscreen" UI down a row, and nothing can repair it afterwards. Those reasons hold today with no other feature involved. A stable origin plus a renderer-owned hit map is also what any future targeted mouse input would need — recorded in the superseded application-API record and preserved here — but that is a beneficiary of this decision, not its premise: the viewport pays for itself even if targeted pointer input never returns. Treating Fullscreen as an owned viewport prevents tall content or external coordinated logs from scrolling the alternate buffer and makes row diffing well-defined. Absolute row replacement reduces ordinary output without changing that contract.
...
- **Why:** a composable used in the wrong place is usually a bug, and a thrown error names it at
  ...
  not a missing context: inside a valid tree, `useLayoutSize().height` may be `Infinity`, while
  `useBoxMetrics().hasMeasured` distinguishes unavailable measurement from a real zero-sized Box.
...
- The internal host-node type is **`TuiNode`** (`TuiContainer | TuiTextLeaf | TuiComment`), not
  Ink's DOM-emulation `DOMElement` (`nodeName` / `attributes` / `childNodes`). vue-tui keeps a
  native host tree, but `useBoxMetrics()` accepts only a direct same-app Vue `Box` ref and does
  not expose or accept `TuiNode` on the supported public surface.
