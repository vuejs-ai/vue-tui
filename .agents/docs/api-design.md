...
[`useInput`](../../packages/runtime/src/composables/useInput.ts) attaches a non-reusable lease to the app's private typed input registry. F3.1–F3.2 decode, structurally frame, and semantically normalize physical input once before the app controller sees one shared immutable fact; F3.3 captures current route leases per framed fact, so a later component cannot inherit split input while unrelated persistent handlers remain and parser-distinct facts route identically whether Node groups their bytes or not. F3.4 defines a private route-plan model that separates action, semantic continuation, delayed defaults, and external permission. The live controller now captures an atomic selected boundary and supplied focus path plus recipient activation leases at fact start, resolves them once, and runs that policy under Inline and Fullscreen. The current hook still receives only a cached Ink-shaped projection; when no explicit private topology is selected, it runs before delayed Ctrl+C, Escape and Tab defaults but cannot prevent them. Focus registration in [`useFocus`](../../packages/runtime/src/composables/useFocus.ts) still does not make its component the owner of subsequent key or paste events; applications manually feed `isFocused` or another state into `useInput({ isActive })` until F4 connects logical focus and rendered lifetime to the already-proved route.

This is adequate for simple applications. The private runtime now defines and proves priority, semantic continuation, modal isolation, delayed-default prevention, and external-owner fallthrough, but no public application surface yet exposes those choices or connects logical focus. [Issue #250](https://github.com/vuejs-ai/vue-tui/issues/250) is direct consumer evidence: several setup-time `useInput` registrations remain alive even when template `v-if` branches change, and manual `isActive` coordination becomes easy to miss.
