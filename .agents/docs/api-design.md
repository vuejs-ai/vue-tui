...
Two shipped APIs were deliberately not promoted into the final picture merely because they existed. `<Static>` has honest terminal-history semantics inline but cannot provide the same persistence full-screen, so its eventual common or inline-specific placement, identity contract, and accepted host matrix remain open in the [Runtime closure matrix](./runtime-foundation-closure.md#finite-closure-matrix). Its decision-independent output transaction is repaired locally at `dd14295`: preparation no longer advances the cursor, a normal write accepts once, a throwing write is not retried, and acceptance uses the prepared item prefix so a synchronous append is not skipped. The former root `useMouseInput()` represented terminal-wide raw mouse rather than element-targeted delivery and was removed during the F6 clean-slate cutover.
