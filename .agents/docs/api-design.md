...
> **Status:** unstamped proposed design program. This record identifies the next product-design layer, the evidence it must satisfy, and the order of work. It records the accepted one-`createApp`, optional-`mode`, default-Inline mount model, the component/composable boundary, and the completed F1 readonly render-session foundation across live, deterministic-test, and string hosts. F2 rendered-target lifetime is Active. The completed implementation is not a VOUCHED stamp, and this record does not select the candidate pointer name or signature, target-ref and event details, target disposition of current pointer and input APIs, a component catalog, or a 1.0 surface.
