...
The remaining gaps are the later lifecycle and publication checkpoints:

- The live mount now uses optional `mode`, defaults omission to an Inline request, rejects old mode/interactivity/`debug` fields before mutation, and resolves one internal session. Deterministic and string hosts provide the same verified authority, but the selected public [`useRenderSession()` projection](./render-session.md) remains unexported until F1.8 because lifecycle facts are not complete.
...
- Full-screen restoration on unmount, exit, and signals, plus the direct-write bypass, are recorded current behavior. F1.7 now owns the target suspension, continuation, final-output, fatal-error durability, and complete restoration semantics specified in the matrix; they remain unimplemented and unexposed through public session facts until that checkpoint closes.
...
1. How are the selected readonly session facts completed for remaining lifecycle behavior without diverging from the implemented live, test, and string authorities?
2. How are the accepted suspension, final-output, fatal-error, and direct-write contracts implemented and exposed without application code reading private state?
3. Which tests and package gates close F1 before the internal session becomes public?
4. Which primitives and components have genuinely shared semantics, which expose capability-dependent behavior, and which should clearly support only one rendering mode?
