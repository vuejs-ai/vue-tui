...
The completed [F5.1 audit and F5.2 proposal](./semantic-geometry-and-caret.md) demonstrate multi-owner cleanup, hidden and clipped targets, wide-glyph edges, resize, non-TTY output, screen-reader output, nested virtual Text, and host-lifetime failures, then select direct Vue targets, fragmented atomic geometry, local rendered cells, and explicit focus eligibility. Logical focus, collection active item, text insertion point, selection, terminal caret, and pointer target remain distinct concepts even when a component coordinates them. Current public code still has the disconnected surfaces until the active implementation replaces them.
