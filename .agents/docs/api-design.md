...
[`useInput`](../../packages/runtime/src/composables/useInput.ts) registers a listener on a shared emitter. F3.1–F3.2 now decode, structurally frame, and semantically normalize physical input once before the app controller sees one shared immutable fact, but the current hook remains a cached Ink-shaped projection. The stdin controller performs Tab or Escape focus behavior and then broadcasts the fact to every active listener in [the app render loop](../../packages/runtime/src/render.ts); each current `useInput` listener sees only that projection. Focus registration in [`useFocus`](../../packages/runtime/src/composables/useFocus.ts) does not make its component the owner of subsequent key or paste events; applications manually feed `isFocused` or another state into `useInput({ isActive })`.
