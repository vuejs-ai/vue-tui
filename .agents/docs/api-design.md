...
[`useInput`](../../packages/runtime/src/composables/useInput.ts) attaches a non-reusable lease to the app's private typed input registry. F3.1–F3.2 decode, structurally frame, and semantically normalize physical input once before the app controller sees one shared immutable fact; F3.3 captures the current route leases when the physical event begins, so a later component cannot inherit split input while unrelated persistent handlers remain. F3.4 now has a private executable route-plan model that separates action, semantic continuation, delayed defaults, and external permission while selecting a modal or pane path before callbacks, but the live controller does not use it yet. The current hook still receives only a cached Ink-shaped projection, and the stdin controller still performs Tab or Escape focus behavior before broadcasting to the frozen active recipients in [the app render loop](../../packages/runtime/src/render.ts). Focus registration in [`useFocus`](../../packages/runtime/src/composables/useFocus.ts) does not make its component the owner of subsequent key or paste events; applications manually feed `isFocused` or another state into `useInput({ isActive })` until live route integration and F4 select that ownership model.
