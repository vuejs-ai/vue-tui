...
[`MountOptions`](../../packages/runtime/src/render.ts) contains rendering-mode and lifecycle choices beside lower-level renderer and scheduler controls. [`useApp`](../../packages/runtime/src/composables/useApp.ts) exposes exit and flush but not effective Inline/Fullscreen mode, output cadence, or terminal capability. The internal session resolves those facts once across live, deterministic-test, and string hosts, but a pure component still cannot adapt through a supported public fact until F1.8 exports the projection.

[`renderToString`](../../packages/runtime/src/render-to-string.ts) now provides a truthful string/document session, isolated inert terminal streams, no-op focus/input/animation mechanics, and explicitly unavailable app lifecycle operations. Public rendering is fixed visual; the internal helper is fixed screen-reader; neither reads process terminal state. The session remains internal until F1.8, so ordinary application components still cannot use the selected public facts API yet.
...
The exact runtime value exports are guarded, and F1.5 adds focused public-type and removed-option guards for `@vue-tui/testing`, but public types remain selectively checked and `components`, `testing`, and `vite` do not share one exhaustive whole-package policy. Several composables expose named `UseXReturn` types while `useFocus`, `useFocusManager`, `useWindowSize`, and `useCursor` return anonymous structures.

`@vue-tui/testing` now directly models Inline or Fullscreen requests, visual or screen-reader presentation, live or teardown update cadence, TTY or stream output, TTY or non-TTY input, and deliberate dimensions. `RenderResult` exposes the same component-visible session, structured `{ dynamic, staticOutput }` frames, a separate xterm-emulated screen with active buffer, viewport, scrollback, cursor, resize, and deterministic `suspend()`/`resume()` controls, plus idempotent disposal. `unmount()` retains the restored screen for assertions; `dispose()` and automatic cleanup release every host resource, after which emulator, resize, suspension, continuation, input, and flush operations fail clearly while `lastFrame()`, frames, and session facts remain inspectable. Removed renderer booleans fail before setup, terminal output never has to be reverse-engineered into frames, and console patching is disabled. Public export completeness and the eventual stable testing surface remain F1.8 work rather than a reason to weaken the modeled host.
...
The remaining gap is the F1.8 publication checkpoint; the lifecycle rows below are completed implementation evidence:

- The live mount now uses optional `mode`, defaults omission to an Inline request, rejects old mode/interactivity/`debug` fields before mutation, and resolves one internal session. Deterministic and string hosts provide the same verified authority, and F1.7 completed the lifecycle facts. The selected public [`useRenderSession()` projection](./render-session.md) remains unexported until F1.8 completes its supported API, type, documentation, consumer, package, and acceptance gates.
...
The accepted finite current-versus-target answers, F1.2 mount contract, Inline-overflow PTY comparison, and completed F1.7 lifecycle behavior are tracked in [rendering-mode-matrix.md](./rendering-mode-matrix.md). The no-clear Inline invariant, application-side escape-hatch requirement, and experimental API-stability policy are vouched. F1.3 selected the unstamped readonly [`useRenderSession()` contract](./render-session.md), and F1.4–F1.7 completed its private authority and behavior. The questions below define F1.8's remaining packet scope rather than a second active backlog.

1. How are the selected readonly session facts exported without exposing the mutable internal service or creating a second source of truth?
2. How are `useWindowSize()` and `useIsScreenReaderEnabled()` removed completely from code, exports, docs, examples, and packed types while repository consumers move to the shared session projection?
3. Which public/type, repository, PTY, visual, restoration, package, clean-consumer, fresh-CI, and independent-review gates close F1?
4. Which primitives and components have genuinely shared semantics, which expose capability-dependent behavior, and which should clearly support only one rendering mode after the public session is available?
