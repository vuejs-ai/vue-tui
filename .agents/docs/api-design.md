...
### Focus and geometry are connected; the terminal caret remains disconnected
...
[`useElementGeometry`](../../packages/runtime/src/composables/useElementGeometry.ts) now follows a normal Vue target through F2 and publishes one atomic paint-derived result with explicit availability, full parent and render-surface bounds, exact rendered fragments, and clipping. The former scalar Yoga hook and imperative measurement API are removed directly, and ScrollBox consumes the replacement. [`useCursor`](../../packages/runtime/src/composables/useCursor.ts) still accepts targetless output-origin coordinates and gives every composable instance cleanup authority over one app-global slot. [`useDraggable`](../../packages/runtime/src/composables/useDraggable.ts) still uses a component ref for pointer behavior, and mouse events expose a clipped Fullscreen hit rectangle. A reusable editor can now observe coherent rendered geometry but still cannot request a focus-eligible element-local terminal caret.

The completed [F5 audit, proposal, private geometry authority, and public geometry cutover](./semantic-geometry-and-caret.md) demonstrate multi-owner cleanup, hidden and clipped targets, wide-glyph edges, resize, non-TTY output, screen-reader output, nested virtual Text, and host-lifetime failures, then implement direct Vue targets and fragmented atomic geometry. The next F5 unit joins an element-local caret request to an exact F4 focus handle and lets one arbiter drive the existing mode writer. Logical focus, collection active item, text insertion point, selection, terminal caret, and pointer target remain distinct concepts even when a component coordinates them.
