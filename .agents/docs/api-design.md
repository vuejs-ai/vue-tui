...
The completed [semantic geometry and caret audit](./semantic-geometry-and-caret.md) establishes the F5 mechanism without selecting names: renderer paint is the authority for one atomic geometry generation, a caret owner declares an element-local rendered insertion cell, one arbiter selects an eligible owner, and the mode writer performs the final render-root-to-terminal translation. Inline and Fullscreen therefore share semantic geometry and caret ownership; they differ only in the final physical output mapping. Parent-relative Yoga reads, targetless output-origin coordinates, focus state, text insertion state, selection, and pointer targeting remain separate concepts.
...
### Focus is connected to routing; geometry and the terminal caret remain disconnected

Completed F4 replaces the former flat string-ID registry with opaque ref-bound targets, rendered eligibility, scopes, traps, centralized restoration, and exact F3 focused delivery. It deliberately exposes no layout, text insertion, selection, terminal-caret, or pointer state.

[`useCursor`](../../packages/runtime/src/composables/useCursor.ts) still accepts targetless output-origin coordinates and gives every composable instance cleanup authority over one app-global slot. [`useBoxMetrics`](../../packages/runtime/src/composables/useBoxMetrics.ts) exposes independently updated parent-relative Yoga values, while `measureElement` accepts `unknown` and returns only width and height. [`useDraggable`](../../packages/runtime/src/composables/useDraggable.ts) uses a component ref for pointer behavior, and mouse events expose a clipped Fullscreen hit rectangle. A reusable editor still cannot derive a current visible rectangle and terminal caret through one coherent rendered-element contract.

The completed [F5.1 audit](./semantic-geometry-and-caret.md) demonstrates multi-owner cleanup, hidden and clipped targets, wide-glyph edges, resize, non-TTY output, screen-reader output, nested virtual Text, and host-lifetime failures. Logical focus, collection active item, text insertion point, selection, terminal caret, and pointer target remain distinct concepts even when a component coordinates them.
