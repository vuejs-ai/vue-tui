...
**Active checkpoint:** F1.6 — make the accepted Inline ownership contract true. Inline must never erase terminal history that existed before the application; its replaceable dynamic region must stay within rows the terminal can still address; committed history must become immutable; resize and coordinated output must re-establish a bounded live region without claiming a fixed origin; direct output remains outside the guarantee; and an application that deliberately wants destructive main-screen behavior retains an explicit escape hatch. The exact overflow projection and supported escape-hatch shape must be selected from real-PTY evidence and kept distinct from Fullscreen's fixed viewport.
...
| F1.6       | **Active** | Implement non-destructive Inline row ownership, exact overflow presentation, history commit behavior, resize/external-output boundaries, and the accepted application escape hatch.                                                                                                                                                                      |
...
| F1    | Rendering-mode session contract | **Active** | Both modes are first-class. F1.2 settled the mount/host policy, F1.3 selected `useRenderSession()` plus `useLayoutSize()`, F1.4 completed their private live behavior source, and F1.5 completed deterministic-test and string-host authority. F1.6 now owns the accepted Inline history and row-bound contract.                             | Live, deterministic-test, and string hosts share authoritative session facts. Testing uses finite host axes, structured render observation, xterm screen emulation, and deterministic cleanup; string rendering uses fixed presentations, isolated inert streams, and explicit unavailable operations; `debug` is removed. Inline still has a destructive overflow clear and unbounded layout rows, which F1.6 must replace. | Every capability and mode-specific API needs one truthful answer for what surface is requested, what became effective, and which environment is running.                                                    |
