...
| F1    | Rendering-mode session contract | **Active** | One `createApp`, public target term `mode`, inline compatibility default, and both modes first-class are selected. Requested versus effective mode, fallback, public environment facts, and migration remain open.                                                                                                 | The shipped mount API still uses `fullscreen` and deprecated `alternateScreen`; testing and static-render APIs do not expose the complete mode matrix.                          | Every capability and mode-specific API needs one truthful answer for what surface is requested, what became effective, and which environment is running.                                                    |
