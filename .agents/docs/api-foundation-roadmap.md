...
**Active checkpoint:** F1.4 — implement the accepted clean-slate live mount surface and one authoritative internal session resolver. This creates the behavior source that the selected public projection will later expose; it does not export `useRenderSession()` while test/string hosts and Inline bounds would still contradict that contract.
...
F1.4 is complete when the accepted mount input, internal live-host resolution, actual terminal behavior, types, examples, package consumer, and every applicable live matrix row agree. The internal session may report `layout.rows: null` for the still-unbounded Inline implementation; it must not claim the target row bound before F1.6. F1.4 does not yet redesign deterministic test controls, expose the string-host session, fix Inline ownership/overflow, or finish suspend/error/final-output lifecycle work.
...
| F1.4       | **Active** | Implement the clean-slate live mount resolver and internal session service; do not publish a host-incomplete projection.                                                                                                                                   |
| F1.5       | Queued     | Replace implicit debug/fake-stream construction with finite production-like test controls, separate content frames from the emulated screen, provide the same session to both string renderers, and reject hidden public screen-reader-option passthrough. |
...
| F1    | Rendering-mode session contract | **Active** | Both modes are first-class. F1.2 settled the mount/host policy, and F1.3 selected `useRenderSession()` plus `useLayoutSize()` with discriminated mode resolution, orthogonal output facts, separate terminal/layout dimensions, and semantic availability. F1.4 implements their live behavior source without exporting a partial public view. | The shipped mount uses old booleans, the screen-reader/Fullscreen combination is a hybrid defect, Inline overflow violates the target, and test/string hosts lack the model.    | Every capability and mode-specific API needs one truthful answer for what surface is requested, what became effective, and which environment is running.                                                    |
