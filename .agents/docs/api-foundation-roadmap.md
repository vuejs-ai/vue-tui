...
**Active checkpoint:** F3 — establish normalized input facts and routing before publishing a new event surface. F3.1 gives each physical stdin one serialized byte ingress, owns Kitty replies, parses structural framing once, multicasts by event-start app generation, and reconciles raw, paste, mouse, listener, and flow ownership across re-entry, suspension, failure, and teardown. F3.2 now normalizes every structural event once into one shared immutable key, text, paste, pointer, or uninterpreted fact; retains complete Kitty alternate-key, modifier, phase, text, functional-key, and source identity; preserves valid but unsupported SGR reports; and confines the current hooks to cached edge projections. Paste-contained control and protocol-like content no longer executes defaults or disappears. The next unit adds handler or route generations before testing priority, continuation, default prevention, and external-owner fallthrough. Public names remain unselected until those competing semantics are measured through the coding-agent composer/approval journey plus a finder or terminal-workbench journey.
