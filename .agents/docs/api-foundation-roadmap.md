...
**Active checkpoint:** F1.2 — define and obtain maintainer acceptance for the exact `mode` mount option and legacy compatibility contract. This is the only planned API task currently in progress.

F1.1 is Done. The accepted [rendering-mode and host behavior matrix](./rendering-mode-matrix.md) covers every live, stream, string, test, screen-reader, input, resize, HMR, suspension, clean-exit, fatal-error, and restoration class. Its committed real-PTY comparison demonstrates why a non-destructive renderer must bound mutable inline rows, why `Static` and application-controlled bounded views cover different longer-content needs, and why full-screen is the mode for arbitrary viewport repaint. A pre-mount clear proves that an explicit application escape is feasible without selecting its final supported API. The production fix, exact clipping presentation, and escape-hatch API remain later implementation and API work.

F1.2 must produce one finite option and migration table covering:

- the exact `mode?: "inline" | "fullscreen"` type, omission, default, and finite equivalence classes for invalid JavaScript values;
- the compatibility meaning and precedence of current `fullscreen` and deprecated `alternateScreen` for every true, false, omitted, and conflicting combination;
- whether conflicts throw, warn, or resolve deterministically, including when validation occurs;
- the deprecation warning policy, compatibility window, removal target, and documentation migration;
...
F1.2 is complete when the option table contains no implicit combination, recent consumer usage and package-version constraints support the migration, the maintainer accepts every compatibility or breaking choice, and the exact contract is reflected in the design records. It does not implement the migration or choose the public session composable.
...
| F1    | Rendering-mode session contract | **Active** | One `createApp`, public target term `mode`, inline compatibility default, both modes first-class, and the finite behavior matrix are accepted. F1.2 owns the mount compatibility contract; public session facts remain later work.                                                                                 | The shipped inline overflow path still violates the target; the mount API uses `fullscreen` and deprecated `alternateScreen`; test and static hosts lack the complete matrix.   | Every capability and mode-specific API needs one truthful answer for what surface is requested, what became effective, and which environment is running.                                                    |
