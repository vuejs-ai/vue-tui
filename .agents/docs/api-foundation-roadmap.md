...
The accepted [rendering-mode and host behavior matrix](./rendering-mode-matrix.md) now contains the exhaustive input-validation and host-resolution tables. Normal `debug: false` non-TTY output defaults to committed Static bytes plus the latest dynamic frame at teardown, while an explicit live-update override may emit relative or linear stream updates without acquiring a terminal mode. Input capability remains independent from this stdout policy. The production Inline fix, exact clipping presentation, history API, escape-hatch API, mount replacement, and complete test-host model remain later F1 work.

**Active checkpoint:** F1.3 — define the minimal readonly session-fact API that lets application code distinguish requested mode, effective mode or no acquired mode, render host and surface, live-update policy, screen-reader path, dimensions, stable-origin availability, hit-map availability, and detectable fallback without inspecting mount options, process globals, or private runtime state.

F1.3 must:

- map every accepted host-table row to finite observable values without collapsing requested and effective mode;
- choose a Vue-native readonly and reactive public shape, including its lifetime and use outside component setup;
- state the retain, replace, or remove disposition of overlapping facts currently exposed by `useApp`, `useWindowSize`, and `useIsScreenReaderEnabled`;
- cover live TTY, non-TTY, screen-reader, deterministic-test, and synchronous-string hosts without pretending every host mounts a terminal session;
- include template and TSX examples and a test-control picture without predesigning pointer, focus, or input-routing operations.

F1.3 is complete when one public candidate has complete semantics for every accepted host row; requested and effective mode cannot be confused by its types or runtime values; immutable and reactive facts have explicit lifetimes; affected current APIs have an explicit target disposition; examples type-check; and the design, matrix, roadmap, and autonomous live objective agree. It does not implement the accepted mount replacement.
...
| F1    | Rendering-mode session contract | **Active** | Both modes are first-class. One `createApp`, optional public `mode` with an Inline default, fail-fast removed keys, and Ink-like non-TTY output are accepted unstamped target decisions. F1.3 owns the readonly session facts.                                                                                                               | The shipped Inline overflow path still violates the target; the mount API uses `fullscreen` and deprecated `alternateScreen`; test and static hosts lack the complete matrix.   | Every capability and mode-specific API needs one truthful answer for what surface is requested, what became effective, and which environment is running.                                                    |
