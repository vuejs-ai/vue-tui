...
**Active foundation:** F4 — logical focus and focus scopes.
...
4. **Completed public contract:** the [public input contract](./input-routing.md#accepted-public-input-contract) replaces `useInput` with one all-run application-global layer over normalized facts, removes `usePaste`, public `Key`, and `exitOnCtrlC`, retains the vouched raw stdin, requires a synchronous four-result handler, publishes an independent semantic-input availability query, fails fast when a registration becomes active on an unavailable live host, keeps inactive and string-host registration inert, and leaves target/local/external attachment to F4. Rejected return-control, query-location, unavailable-host, Ctrl+C, numeric-global-priority, and low-level-topology alternatives remain as decision history.

**Active checkpoint:** F4 — define logical focus and focus scopes on top of F2 rendered-target lifetime and F3 normalized routing. The first unit is evidence and proposal work, not public implementation: audit the flat string-ID focus registry and all real consumers; reproduce the coding-agent approval overlay, finder, independent-region, removal, and restoration journeys; recheck peer focus, scope, traversal, trap, and event-attachment mechanisms; then present one complete contract with dispositions for `useFocus` and `useFocusManager`. Do not select or publish target/local/external APIs until that contract is accepted.
...
| F3    | Normalized input and routing    | Done       | Normalized public facts, a required route result, one all-run application-global hook, an independent availability query, fail-fast live activation, inert inactive and string-host registration, and F4-owned target/local/external attachment are settled. `usePaste`, public `Key`, raw-mode operations and policy, and `exitOnCtrlC` are removed.                                | One shared ingress, immutable fact, fact-start routes, independent globals, atomic selected generations, preventable defaults, transactional demand, normalized PTY fallthrough, all public/host/package surfaces, and every closure gate are implemented and verified. | Routing is required by every interactive scenario and now defines delivery semantics before focus or pointer specializes them.                                                             |
| F4    | Logical focus and focus scopes  | **Active** | The app runtime should own one effective logical-focus target, with rendered lifetime, traversal, traps, and restoration. Exact attachment, scope, and disposition of current focus APIs remain open; the next unit gathers evidence and prepares the complete contract rather than publishing an API.                                                                               | The current focus registry is flat, string-ID based, setup-scoped, and separate from normalized input delivery and rendered elements. F2 and F3 supply the lifetime and event/routing mechanisms that the accepted F4 contract must reuse.                              | Focus can select one owner inside the defined input route instead of inventing its own event model; it immediately unlocks editors, finders, overlays, and keyboard-driven scrolling.      |
...
Under the accepted [F3/F4 attachment boundary](./input-routing.md#attachment-boundary-publish-one-global-hook-in-f3-choose-local-attachment-in-f4), the active-region, component, ancestor, and external-owner bullets require an executable route plus one reusable public result contract, not a public API for applications to select those recipients manually. F4 then publishes the target/local/external attachment while keeping F3's order and outcomes. The rejected low-level selection alternative remains in the decision record so the roadmap does not silently count private topology as a finished ordinary authoring API.
