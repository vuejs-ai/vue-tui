...
The accepted [rendering-mode and host behavior matrix](./rendering-mode-matrix.md) now contains the exhaustive input-validation and host-resolution tables. Normal non-TTY output defaults to committed Static bytes plus the latest dynamic frame at clean teardown, while an explicit live-update override may emit relative or linear stream updates without acquiring a terminal mode. Input capability remains independent from this stdout policy. F1.4 implemented the mount replacement and live resolver, F1.5 completed the deterministic-test and string-host authority, F1.6 completed the bounded non-destructive Inline ownership contract, and F1.7 completed the lifecycle behavior. F1.8 is Active and owns public projection plus final F1 closure.
...
| F1.8       | **Active** | Export `useRenderSession()` and `useLayoutSize()`, remove superseded hooks, close remaining option dispositions and exhaustive API/type/PTY/CI/package gates, then mark F1 Done and activate F2.                                                                                                                                                         |
...
| F1    | Rendering-mode session contract | **Active** | Both modes are first-class. F1.2 settled the mount/host policy, F1.3 selected `useRenderSession()` plus `useLayoutSize()`, F1.4 completed their private live behavior source, F1.5 completed deterministic-test and string-host authority, F1.6 completed Inline history and row-bound ownership, and F1.7 completed lifecycle behavior.     | Live, deterministic-test, and string hosts share authoritative session facts. Visual Inline enforces bounded non-destructive ownership, and every live surface now has exact mount, teardown, suspend/continue, clean-exit, and fatal-error behavior. F1.8 public projection and final F1 acceptance remain. | Every capability and mode-specific API needs one truthful answer for what surface is requested, what became effective, and which environment is running.                                                    |
