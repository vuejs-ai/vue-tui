...
**Active checkpoint:** F3 — establish normalized input facts and routing before publishing a new event surface. Audit the current stdin controller, parser, protocol-reply filters, `useInput`, `usePaste`, `useMouseInput`, focus delivery, global exit handling, and external PTY handoff. Reproduce information loss, duplicate parsing, broadcast ambiguity, and fallthrough gaps through a coding-agent composer/approval journey plus a finder or terminal-workbench journey. Begin with behavioral vocabulary, red tests, and internal routing experiments; do not select public names until the competing handled/continue/default/external-owner semantics are measured.
...
| F3    | Normalized input and routing    | **Active** | Input must be parsed once, preserve the key/text/paste facts the parser already knows, and define handled/continue plus priority/fallthrough. The exact event API and disposition of current input APIs remain open.                                                                                                                                                                 | `useInput` broadcasts to every active listener and exposes a reduced `Key`; protocol replies, application handlers, and external PTY fallthrough do not share one public route. The active entry step is an audit, vocabulary, and red tests. | Routing is required by every interactive scenario and should define delivery semantics before focus or pointer specializes them.                                                                   |
