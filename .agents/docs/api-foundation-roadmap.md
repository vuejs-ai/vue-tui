...
**Active checkpoint:** F3 — establish normalized input facts and routing before publishing a new event surface. F3.1 gives each physical stdin one serialized byte ingress, owns Kitty replies, parses structural framing once, multicasts by event-start app generation, and reconciles raw, paste, mouse, listener, and flow ownership across re-entry, suspension, failure, and teardown. F3.2 normalizes every structural event once into one shared immutable key, text, paste, pointer, or uninterpreted fact; retains complete Kitty alternate-key, modifier, phase, text, functional-key, and source identity; preserves valid but unsupported SGR reports; and confines the current hooks to cached edge projections. F3.3 now adds non-reusable typed route leases to the event-start snapshot, so a replacement cannot inherit split key, paste, or pointer input while persistent routes and other apps retain delivery; ref updates, re-entry, bootstrap, suspension, paste fallback, and mouse classification have explicit control behavior. The next unit tests priority, continuation, default prevention, and external-owner fallthrough. Public names remain unselected until those competing semantics are measured through the coding-agent composer/approval journey plus a finder or terminal-workbench journey.
...
| F3    | Normalized input and routing    | **Active** | Input must be parsed once, preserve the key/text/paste facts the parser already knows, and define handled/continue plus priority/fallthrough. The exact event API and disposition of current input APIs remain open.                                                                                                                                                                 | F3.1 centralizes byte decoding, structural framing, protocol replies, ordered multi-app delivery, and lifecycle ownership per stdin. F3.2 adds one shared immutable semantic fact and cached legacy-hook projection. F3.3 adds event-start non-reusable route leases and stable paste/mouse classification. Priority, default control, raw-byte external-owner recovery, and fallthrough remain next. | Routing is required by every interactive scenario and should define delivery semantics before focus or pointer specializes them.                                                                   |
