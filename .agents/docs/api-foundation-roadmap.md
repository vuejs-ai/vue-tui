...
The accepted [rendering-mode and host behavior matrix](./rendering-mode-matrix.md) now contains the exhaustive input-validation and host-resolution tables. Normal non-TTY output defaults to committed Static bytes plus the latest dynamic frame at teardown, while an explicit live-update override may emit relative or linear stream updates without acquiring a terminal mode. Input capability remains independent from this stdout policy. F1.4 implements the mount replacement and live resolver; F1.5 completes the deterministic-test and string-host authority; and F1.6 completes the bounded, non-destructive Inline ownership contract. F1.7 is Active and owns the remaining lifecycle behavior before the session facts can be published.
...
**Active checkpoint:** F1.7 — make lifecycle behavior match the accepted surface contract. External suspension must restore terminal state before `SIGTSTP`, reacquire the same resolved surface after `SIGCONT`, refresh dimensions, and repaint. Clean exit must retain Inline output, restore and discard the Fullscreen viewport without replaying it, and finish final-output streams once. Fatal errors must remain durable: Inline and stream hosts leave an error in their output, while Fullscreen first restores the main screen and then writes the error to stderr. Coordinated output follows each surface's ownership rules; returned raw streams remain an explicit bypass. Every path must restore only the terminal modes it acquired, including mount failure, unmount, app exit, signals, suspension, continuation, and fatal error.
...
| F1.7       | **Active** | Implement and verify suspension/resume, clean exit, fatal-error durability, coordinated/direct output, and remaining terminal restoration semantics across every host.                                                                                                                                                                                   |
...
| F1    | Rendering-mode session contract | **Active** | Both modes are first-class. F1.2 settled the mount/host policy, F1.3 selected `useRenderSession()` plus `useLayoutSize()`, F1.4 completed their private live behavior source, F1.5 completed deterministic-test and string-host authority, and F1.6 completed Inline history and row-bound ownership. F1.7 now owns lifecycle completion.    | Live, deterministic-test, and string hosts share authoritative session facts. Visual Inline enforces maximum rows, hard clipping, immutable history commits, and non-erasing resize recovery; testing and real PTYs model those results. F1.7 lifecycle behavior and F1.8 public projection remain. | Every capability and mode-specific API needs one truthful answer for what surface is requested, what became effective, and which environment is running.                                                    |
