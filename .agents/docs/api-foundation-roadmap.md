...
**Active foundation:** F2 — rendered-target lifetime.
...
**Active checkpoint:** F2 — audit rendered-target lifetimes and establish the internal contract before publishing any new target API. Start from the lifetime class behind issue #250 and every existing ref-bound registration. Reproduce null, insertion, `v-if` removal, retargeting, unmount, scope disposal, and HMR failures with focused tests; then make two internal registration adapters use one seam without defining focus, geometry, caret, or pointer semantics early.

### Completed F1 sequence
