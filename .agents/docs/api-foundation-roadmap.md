...
**Active checkpoint:** F3 — establish normalized input facts and routing before publishing a new event surface. F3.1 now gives each physical stdin one serialized byte ingress, owns Kitty replies, parses structural framing once, multicasts by event-start app generation, and reconciles raw, paste, mouse, listener, and flow ownership across re-entry, suspension, failure, and teardown. The next unit normalizes lossless semantic facts once and adds handler or route generations before testing priority, continuation, default prevention, and external-owner fallthrough. Public names remain unselected until those competing semantics are measured through the coding-agent composer/approval journey plus a finder or terminal-workbench journey.
