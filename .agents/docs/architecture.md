...
| `text/`   | Measurement, wrapping, truncation, and parsing user strings (including ANSI) into styled runs and the sequences written around them, produced once per content revision | `frame/`   |

Above both paths:

| Directory  | Owns                                                                                                                                             | May import                                                                                |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------- |
| `api/`     | Package entries: `createApp`, `MountOptions`, `renderToString`, the two `internal/*` seams                                                       | everything, plus `vue`, and `node:stream` as types only                                   |
...
Plain data holding one picture's worth of cells, plus its size. It stores them in parallel arrays — graphemes, widths, packed colours, an attribute bitmask, the unmodelled SGR and the links — rather than an object grid, so a blank cell costs what a styled one costs; `benchmarks/runtime/renderer.bench.ts` measures that layout. Four properties matter.

- Two frames are alive at once, the current one and the previous one. The shared `blankCell` in `frame/cell.ts` is the one unpainted cell every frame starts from and the one the painter writes into a blanked column, so a `Cell` object exists only where one is read or written, never one per cell per frame.
...
| `Session`            | The Vue app, the node tree, `LayoutEngine`, `Painter`, `Surface`, terminal leases, the scheduler, `InputDispatcher`, focus ownership, geometry registration and exit settlement | One commit is layout → paint → `surface.present(frame)`. The session supplies the available size: the terminal's columns, and the height constraint the surface's kind returns for the rows: `exact` for Fullscreen, `at-most` for Inline and Document, `unbounded` when the rows are unknown. Its lifecycle is a union type whose members carry the facts each state needs: mounting carries whether a suspension arrived before the mount could release the terminal and how far Vue's own mount has got, suspended carries the step the resume transaction has reached, tearing-down carries the state it began from, the step it has reached, what it still owes and the Vue tree it has yet to release, and running and torn-down carry nothing — reaching running means Vue's mount returned. It is a **session** state rather than a process state: `SIGTSTP`, signal exit and exit codes belong to the node backend. |
...
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `InlineSurface`     | Its own previous frame, its own region and cursor-hidden lease, and the handling for consecutive frames that differ in height |
...
`TerminalBackend` keys acquire/release leases by a mode identifier, and the lease operates the mode itself: it goes on when the first holder arrives and comes back off when the last one leaves. Raw mode, the alternate screen, cursor visibility, synchronized output, bracketed paste and the Kitty protocol are six modes in the current Runtime, so one reference-counted mechanism replaces six separate ownership protocols. The mechanism carries the output transaction with it: a written mode change travels through the session's output gate so it keeps its place among the frames it belongs between, and a signal exit or emergency restore writes it straight to the file descriptor instead. One sweep behind every owner restores what an owner could not, on suspension, teardown and runtime failure alike.

Five of the six are escape sequences on an output this process owns alone, so their ledger lives with the backend that writes them. Raw mode is a line discipline: its device transition is an ioctl on an **input** every session mounted on the same stdin shares, so its registry is process-wide inside `terminal/`, keyed by that input, and every backend on it holds a share of one state. Its lease also defers the restore by one microtask, which is what lets a same-tick replacement inherit raw input rather than switch it off and straight back on; a suspension, a teardown and a failed acquisition ask for the transition immediately instead. Future terminal modes use the same mechanism only after their product behavior is accepted.
...
- The [Runtime benchmark workspace](../../benchmarks/runtime) is where `Frame`'s memory layout is measured, so a change to it argues from numbers.
