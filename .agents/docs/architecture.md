...
| `terminal/` | The `TerminalBackend` interface plus its node and test implementations, mode leases, capabilities, size, backpressure  | nothing (`terminal/node/` may import `node:*`) |
...
| `text/`   | Measurement, wrapping, parsing user strings (including ANSI) into cells; the cache measurement and paint share | `frame/`   |
...
- `diff` is **the only place in the system that answers "what changed"**.
...
| `Painter`            | Nothing                                                                                                                                                                         | Tree + `ComputedLayout` + viewport → `Frame`. No I/O, no strings. Measurement and wrapping caches live in `text/`, shared with `LayoutEngine`.                                                                                                                                                    |
...
An architecture has to stop bad changes rather than rely on memory. The first three are import rules; the last two are type rules.
