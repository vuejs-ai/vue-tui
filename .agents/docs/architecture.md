...
`TerminalBackend` keys acquire/release leases by a mode identifier, and the lease issues the mode itself: the enable bytes when the first holder arrives and the restore bytes when the last one leaves. Raw mode, the alternate screen, cursor visibility, synchronized output, bracketed paste and the Kitty protocol are six modes in the current Runtime, so one reference-counted mechanism replaces six separate ownership protocols. The mechanism carries the output transaction with it: a mode change travels through the session's output gate so it keeps its place among the frames it belongs between, and a signal exit or emergency restore writes it straight to the file descriptor instead. One sweep behind every owner restores what an owner could not, on suspension, teardown and runtime failure alike.

Five of the six are escape sequences and are written this way. Raw mode is a line discipline rather than a sequence, and the terminal it switches is the **input** every session mounted on the same stdin shares, so its lease counts ownership while `session/stdin-controller.ts` still reconciles the shared physical state. Future terminal modes use the same mechanism only after their product behavior is accepted.
