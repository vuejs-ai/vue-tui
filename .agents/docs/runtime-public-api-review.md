...
- vue-tui differs from Ink where Vue semantics and the chosen minimum contract require it: several sibling Static instances use current Vue tree order instead of Ink's single tracked Static node, and mounted identity rather than layout visibility determines eligibility because `v-show` does not unmount a Vue component. Terminal-specific hooks remain inert in both implementations.
