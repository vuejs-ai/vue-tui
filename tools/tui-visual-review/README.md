...
Use `vp run visual:fullscreen-origin -- --scenario foreground-reset` to inspect nested `color="default"` foreground behavior, wrapping, sibling colors, inherited background, literal private-use characters, and shell restoration. Use `--scenario box-text-contract` for the complete Box/Text phase: inspect the reverse wrapped panels, gap and border-edge state, independent foreground/background defaults, modifier cascade, clipping, truncation, hard wrapping, and absolute badge; resize narrower, press `t` to withdraw the explicit gap, padding, border-edge, bold, and dim overrides, then quit with `q` and verify shell restoration. Use `vp run visual:inline-history` to inspect the final Inline committed `Static` history plus newest live tail. Use `vp run visual:v-show` to inspect direct Box, top-level Text, and nested Text hide behavior, hidden-state update, show-again without automatic Focus restoration, explicit `f` reacquisition, measurement, and terminal restoration one observed action at a time.

Use `vp run visual:vite-overlay` for the in-process Vite development path. The target launches the same programmatic Vite server used by the integration suite, avoiding the monorepo's non-runnable Vite+ CLI SSR environment. Observe the running app, introduce a syntax error in `packages/vite/test/fixtures/overlay/src/app.vue`, observe the `Build Error` frame, wait and confirm that the error remains unchanged, restore valid source, wait for the error to disappear, and observe the recovered app. Then press `q` from an observation and verify application exit plus terminal restoration. The transient `[HMR] updated` frame lasts two seconds; deterministic integration tests cover its exact event ordering when an interactive tool round trip is too slow to capture it.
