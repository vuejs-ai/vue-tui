...
- Tests must not implicitly depend on the host environment. Non-TTY environments disable chalk colors, so vitest configs and PTY child envs force `FORCE_COLOR` to keep ANSI output exercised. If a test needs a specific TTY/color behavior, inject it explicitly (fake stream, mount option, child env) rather than relying on the ambient value.
