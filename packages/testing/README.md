...
Output cadence is not an option at all: it follows the host, so a TTY stdout is always live and a non-TTY one is always the final document. Set `stdout` instead.

`RenderOptions` is the contract. TypeScript rejects an unknown key, and `render()` adds no runtime key check on top of the type. The one runtime option check is `debug`, kept only because TypeScript can say the option does not exist but not what to use instead.

### Replacing Ink's `debug` option

`debug` is rejected rather than supported, and nothing was lost with it. In Ink, `debug: true` makes the application write **different bytes**: every commit emits its complete current content with no erase sequences, Static history replays, and clear and console-patching behavior change. That makes frames easy to read out of stdout, but the byte stream you then assert on is one production never produces — so erase behavior, Static settlement, and console coordination become untestable in exactly the mode you turned on to inspect them.

Observation here happens inside the renderer instead, before a frame becomes ANSI, so the application's bytes are untouched:

| You used `debug` to…                      | Use this instead                                                                  |
| ----------------------------------------- | --------------------------------------------------------------------------------- |
| read each committed frame                 | `frames` — structured `ContentFrame` values, no escape-sequence parsing           |
| read the final output                     | `lastFrame()`                                                                     |
| see what the terminal would actually show | `await screen()` — a real xterm emulation of the untouched byte stream            |
| check that output was not mangled         | assert on the emulated screen; the bytes are production-accurate, so this is real |

The last row is the part `debug` could not do at all: with it, the erase and cursor sequences you would want to check were the ones it had switched off.
