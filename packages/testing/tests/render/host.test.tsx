...
test("modeled hosts own deterministic styling instead of inheriting the worker environment", async () => {
  const App = defineComponent(() => () => <Text color="#ff0080">styled</Text>);
  const tty = await render(App);
  const stream = await render(App, { stdout: "stream" });

  try {
    expect(tty.lastFrame({ raw: true })).toBe("\x1b[38;2;255;0;128mstyled\x1b[39m");
    expect(stream.lastFrame({ raw: true })).toBe("styled");
  } finally {
    tty.dispose();
    stream.dispose();
  }
...
test("an explicit color option overrides the modeled stdout capability", async () => {
  const App = defineComponent(() => () => (
    <Text bold color="#ff0080">
      styled
    </Text>
  ));
  const ttyWithoutAnsi = await render(App, { color: false });
  const truecolorStream = await render(App, { color: "truecolor", stdout: "stream" });

  try {
    expect(ttyWithoutAnsi.lastFrame({ raw: true })).toBe("styled");
    expect(truecolorStream.lastFrame({ raw: true })).toBe(
      "\x1b[1m\x1b[38;2;255;0;128mstyled\x1b[39m\x1b[22m",
    );
  } finally {
    ttyWithoutAnsi.dispose();
    truecolorStream.dispose();
  }
});

test("plain output removes authored SGR", async () => {
  const result = await render(() => <Text>{"\x1b[31mraw\x1b[39m"}</Text>, { color: false });

  try {
    expect(result.lastFrame({ raw: true })).toBe("raw");
  } finally {
    result.dispose();
  }
});
...
test("a modeled non-TTY keeps direct stdin bytes available", async () => {
  ...
  const result = await render(App, { stdin: "non-tty" });
...
test("a removed option fails before its accessor or component setup runs", async () => {
  ...
  // `debug` is refused by an own-property test, so its value is never evaluated.
  const options = Object.defineProperty({}, "debug", {
    enumerable: true,
    get() {
      reads++;
      return true;
  ...
  await expect(render(App, options as RenderOptions)).rejects.toThrow(
    'render option "debug" was removed',
  );
  expect(reads).toBe(0);
...
test.each([
  [{ mode: "sideways" }, 'render option "mode"'],
  [{ mode: null }, 'render option "mode"'],
  [{ stdin: "maybe" }, 'render option "stdin"'],
  [{ stdin: null }, 'render option "stdin"'],
  [{ stdout: "file" }, 'render option "stdout"'],
  [{ stdout: null }, 'render option "stdout"'],
  [{ color: "none" }, 'render option "color"'],
  [{ exitOnCtrlC: "yes" }, 'render option "exitOnCtrlC"'],
  [{ retainFrames: "yes" }, 'render option "retainFrames"'],
  [{ columns: 0 }, "render columns"],
  [{ rows: Number.NaN }, "render rows"],
  [{ debug: true }, 'render option "debug" was removed'],
] as const)("invalid render option %# fails before component setup", async (options, message) => {
  let setupRan = false;
  const App = defineComponent(() => {
    setupRan = true;
