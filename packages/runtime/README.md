...
Each mounted instance remains open until its first non-empty eligible output. Only a block represented by non-empty bytes in the current settlement transaction is accepted; an output-free render or an authored hidden Box ancestor leaves the instance open for the latest slot content, while ordinary unmount before output writes no history. Several blocks accepted in one transaction use current Vue host-tree preorder, independent of registration, mount, Yoga visual, or reverse-flex order. Later eligibility appends, accepted history never moves, and remounting creates a new block.

`Static` may appear at the application root, through components or Fragments, or inside ordinary Box structure. Its host stays outside dynamic layout flow and paints its slot as one isolated width-constrained block, so ancestor Box size, padding, flex, and clipping do not shape the history; put layout that belongs to the block inside its slot. A `Static` nested at any depth inside another `Static`, or inside Text or Transform context, is rejected before output. Direct `v-show` on `Static` is unsupported; place it on an authored Box ancestor instead.

After a normally returning write, including backpressure signaled by `false`, Runtime seals the complete represented batch and lets Vue finish replacing every accepted host with its stable comment anchor. Runtime isolates effect-scope cleanup callbacks that would otherwise abort release, then forwards the first captured failure through Vue's normal capture chain after the batch patch completes; watcher invalidation and lifecycle-hook errors already handled by Vue retain their native per-hook timing. Ancestor removal and whole-app teardown settle the same batch, and exit waits for settlement. During teardown, an isolated failure reserves its first-cause position when observed even though Vue reporting remains delayed, so a later ordinary cleanup error cannot replace it. A throwing write is fatal, abandons the represented blocks, and never retries indeterminate bytes. An invalid initial placement consumes the mount attempt but rolls back without Static output; a later invalid reactive insertion follows ordinary Vue update-error handling and an open block can commit after application repair. On non-TTY output, an accepted block appends immediately before the current dynamic document is written once at clean teardown. Effective visual Fullscreen rejects `Static` before Static bytes or a replacement frame are written; keep Fullscreen history in application state, for example with a bounded `ScrollBox`.
