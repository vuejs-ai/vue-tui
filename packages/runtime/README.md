...
const lines = Array.from({ length: 100 }, (_, index) => `Row ${index + 1}`);
// An unbounded host reports Infinity; Box needs a finite height or percentage.
...
Runtime preserves vertical content by supplying `flexShrink: 0` to in-flow children of vertical stacks during layout, because Yoga has no content-based automatic minimum height. A `flexShrink` written on a Box is used as written; the public default remains `1`. Set `overflow="hidden"` on a container whose overflow should not paint outside it.
