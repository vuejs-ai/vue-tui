...
import { createRenderedTargetController } from "../session/rendered-target.ts";
...
function renderStringDocument(
  ...
  const renderer = createRenderer<TuiNode, TuiNode>(
    buildNodeOps({
      ...
      onCommit: () => {},
      invalidateRenderedSubtree: (target) => renderedTargets.invalidateSubtree(target),
  ...
  try {
    ...
    app.provide(RenderedTargetControllerKey, renderedTargets);
    ...
    try {
      ...
      // Best-effort: remaining string-host cleanup must still run.
    }
