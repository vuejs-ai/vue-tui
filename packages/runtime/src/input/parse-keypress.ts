// Parser structure adapted from Ink v7.0.4:
// https://github.com/vadimdemedes/ink/blob/40b3a7578811fd616341ca4e31cc7748aeeff12f/src/parse-keypress.ts

import { kittyModifiers } from "../terminal/kitty-keyboard.ts";

const textDecoder = new TextDecoder();

// Legacy Alt prefixes one Unicode code point with ESC. The input parser waits
// out the ambiguity with CSI/SS3; complete multi-character sequences do not match.
const metaKeyCodeRe = /^\x1b(.)$/su;

const fnKeyRe = /^(?:\x1b+)(O|N|\[|\[\[)(?:(\d+)(?:;(\d+))?([~^$])|(?:1;)?(\d+)?([a-zA-Z]))/;

// Kitty keyboard-protocol query-response (ESC[?Nu). Detection does not cover the
// complete input pipeline: enabled mode skips probing, and auto mode's detector
// and stdin controller both observe the same data event. This parser-level net
// prevents a response from surfacing as user input; the end-to-end Kitty
// query-response tests pin the complete path.
