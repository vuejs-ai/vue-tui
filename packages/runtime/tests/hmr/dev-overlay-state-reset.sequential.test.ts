...
// concurrently. Mirrors bridge-idempotent.sequential.test.ts.
import { afterEach, expect, test, vi } from "vite-plus/test";

// The dynamic import() path must be a string LITERAL so the bundler resolves it
// relative to this file. resetDevState lives in hmr.ts, and render()'s dev block
// calls it — see render.ts's isDevConnected() gate.
