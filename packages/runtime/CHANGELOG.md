...
- Reduced `MountOptions` to streams, mode, presentation, and console patching; reduced `useApp()` to `exit(error?)`; and kept render-flush and exit barriers on the app-owner handle. Runtime now owns terminal acquisition, cadence, failure aggregation, suspend/resume, and restoration through one lifecycle without exposing scheduler or resource controls.
