...
- Replaced Yoga-only `useBoxMetrics()`, imperative `measureElement()`, and the experimental broad `useElementGeometry()` projection with Box-only `useBoxSize()`. It accepts a direct same-app `<Box>` ref and reports only the last accepted full `{ width, height }`; paint fragments, clipping provenance, surface coordinates, and target-lifetime machinery remain internal for caret and mouse behavior. The targetless `useCursor()` remains replaced by focus-bound `useCaret()`, and writers retain their last successful frame and caret baseline when a stream write fails so an identical retry is not dropped.
