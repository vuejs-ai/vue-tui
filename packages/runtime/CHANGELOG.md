...
- Reduced `MountOptions` to streams, mode, console patching, and the default-off `exitOnCtrlC` convenience; reduced `useApp()` to `exit(error?)`; and kept render-flush and exit barriers on the app-owner handle. Runtime now owns terminal acquisition, cadence, failure aggregation, suspend/resume, and restoration through one lifecycle without exposing scheduler or protocol controls. Console patching remains under independent public-API review.
