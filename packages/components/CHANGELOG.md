...
- **`Table`** — responsive bordered tables with inferred or explicitly typed columns, multiline cells, per-column wrapping, alignment, padding, string formatting, and structured header/cell text styles.
...
- **Visibility** — `Newline`, `Spacer`, `Spinner`, `ScrollBox`, and a non-empty `Table` keep a single Box or Text root and inherit host-based `v-show`; a `Table` with neither rows nor explicit columns occupies no layout space, and hidden components remain mounted.
