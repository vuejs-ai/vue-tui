...
test("capacity manifest keeps the fixed J1-J6 workload", () => {
  expect(capacityManifest).toStrictEqual({
    ...
    j6i: {
      ...
      rows: 30,
      volumes: {
        small: { completedRecords: 100, frameUpdates: 200 },
        large: { completedRecords: 1_000, frameUpdates: 2_000 },
      },
      coordinatedEvery: 10,
...
test("the capacity runner executes both J6 volumes at the frozen repetition count", () => {
  ...
  expect(runnerSource).toContain("schemaVersion: 7");
  expect(runnerSource).toContain('basis: "tracked-runtime-lifetimes-v1"');
  expect(runnerSource).toContain("supersedesSchemas: Object.freeze([4, 5, 6])");
...
test("capacity diagnostics exclude retained execution evidence", () => {
  ...
  // Host-tree observation no longer depends on production node-creation hooks.
  expect(hostSource).not.toContain("observeTuiNodeCreations");
  expect(hostSource).not.toContain("trackTuiTree");
  expect(hostSource).toContain('trackLifetimeTarget?.("tui-app", app)');
  const nodeSource = readFileSync(
    new URL("../../runtime/src/host/nodes.ts", import.meta.url),
    "utf8",
  );
  expect(nodeSource).not.toContain("tuiNodeCreationObservers");
  expect(nodeSource).not.toContain("observeTuiNodeCreations");
...
describe("capacity acceptance policy", () => {
  ...
  test("fails when lifetime coverage evidence is incomplete", () => {
    ...
    const incomplete: CapacityWorkerEvidence = {
      ...
      retention: {
        ...
        audits: [
          {
            ...firstAudit,
            observedTargets: { ...firstAudit.observedTargets, "tui-app": 0 },
          },
          ...workload.retention.audits.slice(1),
        ],
      },
