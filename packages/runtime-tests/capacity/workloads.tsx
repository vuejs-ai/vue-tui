...
export const capacityManifest = Object.freeze({
  ...
  j6i: Object.freeze({
    ...
    volumes: Object.freeze({
      small: Object.freeze({ completedRecords: 100, frameUpdates: 200 }),
      large: Object.freeze({ completedRecords: 1_000, frameUpdates: 2_000 }),
    }),
    coordinatedEvery: 10,
...
async function finish(
  ...
  return Object.freeze({
    ...
    assertionCount: assertionCount + 3,
...
async function runJ6Inline(volume: CapacityVolume): Promise<JourneyExecution> {
  ...
  try {
    ...
    for (let update = 1; update <= configuration.frameUpdates; update++) {
      await j6ProducerTurn(() => {
    ...
    assert.equal(
      pump.records.length,
      configuration.frameUpdates / capacityManifest.j6i.coordinatedEvery,
    );
    ...
    assert.equal(generations.at(-1), configuration.frameUpdates);
    assert.ok(
      new Set(generations).size < configuration.frameUpdates + 1,
  ...
  const execution = await finish("j6i", host, [], renderDurations, heartbeat, assertions + 8, {
    volume,
    backpressure: Object.freeze({ ...counts }),
  });
