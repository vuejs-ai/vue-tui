...
// Props the public surface genuinely rejects. Widened-but-public props such as
// `alignContent`, `aspectRatio`, and the per-edge border colors do NOT belong
// here — listing one would forbid first-party code from using supported API. The
// last test in this file keeps both lists honest against the prop declarations.
...
const removedTextProps = new Set(["ariaHidden", "ariaLabel", "class", "style"]);

// Generated and vendored trees are not authored source. Descending into them made
// this scan read every built example bundle once `vp run build` had produced them
// — about 13 MB of .mjs — which blew the 5 s timeout in a full run while the file
// passed alone on an unbuilt tree. Bundled output also embeds Runtime's own prop
// names, so scanning it risks a false positive on top of being slow.
const skippedDirectories = new Set(["node_modules", "dist", "tmp", ".vite", ".turbo", "coverage"]);
...
function sourceFiles(root: string): string[] {
  ...
  return readdirSync(root, { withFileTypes: true }).flatMap((entry) => {
    ...
    if (entry.isDirectory()) {
      return skippedDirectories.has(entry.name) || entry.name.startsWith(".")
        ? []
        : sourceFiles(path);
    }
...
// Reads the prop declaration Runtime derives both `BoxProps`/`TextProps` and the
// closed validator from. The exported components publish `PublicComponent`,
// which deliberately hides Vue's `.props`, so the declaration is the accessible
// source of truth rather than the constructor object.
function declaredPropNames(file: string, declaration: string): string[] {
  const path = join(runtimeSourceRoot, "components", file);
  ...
  visit(source);
  if (!names?.length) throw new Error(`${file}: no \`${declaration}\` object literal found`);
  return names;
}

// The removed-prop lists are hand-maintained, so a prop that is later added back
// to the public surface can be left behind in them. That failure is silent and
// backwards: the scan above would forbid first-party code from using supported
// API. Check both lists against the declaration so the drift fails here instead.
test("the removed-prop lists name only props the public surface rejects", () => {
  const declared: Record<PrimitiveName, string[]> = {
    Box: declaredPropNames("box-props.ts", "boxProps"),
    Text: declaredPropNames("text-props.ts", "textProps"),
  };
  expect(declared.Box).toHaveLength(62);
  expect(declared.Text).toHaveLength(9);

  for (const [component, removed] of [
    ["Box", removedBoxProps],
    ["Text", removedTextProps],
  ] as const) {
    for (const name of removed) {
      expect(declared[component], `${component}.${name} is public, not removed`).not.toContain(
        name,
      );
    }
  }
