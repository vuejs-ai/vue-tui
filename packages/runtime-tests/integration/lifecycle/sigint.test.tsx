...
test.each([
  ...
  const result = await render(App, exitOnCtrlC === undefined ? {} : { exitOnCtrlC });
...
test.each(["inline", "fullscreen"] as const)(
  ...
  async (mode) => {
    ...
    const result = await render(App, { mode, exitOnCtrlC: true });
...
test("exitOnCtrlC true delivers Ctrl+C with any other command modifier", async () => {
  ...
  const result = await render(App, { exitOnCtrlC: true });
