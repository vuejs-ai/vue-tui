...
describe("build output: package.json exports resolve to built files", () => {
  ...
  test("runtime publishes exactly the accepted public and privileged entries", () => {
    ...
    // privileged entries reserved for official packages. Exact-key assertions on
    // the privileged entries live here, in a repository package-boundary test —
    // the external clean consumer must not inventory them.
