...
it("independent public and managed raw owners keep the app alive until managed input exits", async () => {
  ...
  // __READY__ after its initial frame is flushed. Their shared stdin ref keeps
  // the process alive until q reaches the managed route and exits.
  const ps = term("exit-double-raw-mode");

  // This is an absence claim: after __READY__ (resolved internally by the term
  // helper), keep one explicit observation window and prove that ownership did
  // not release merely because the initial render completed.
