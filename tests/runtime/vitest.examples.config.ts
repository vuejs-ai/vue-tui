...
// server or a `node dist/main.js` build — and waits for it to paint. Kept separate from the
// integration and PTY suites because it shells out to `vite`/`node` and needs build-and-launch
// headroom rather than the fast in-process render timings the other configs assume.
export default defineConfig({
  test: {
    include: ["e2e/examples/**/*.test.ts"],
    ...
    // suites under `vp run check` / CI. Generous on purpose; a healthy launch resolves in seconds.
    testTimeout: 60000,
    hookTimeout: 60000,
  },
});
