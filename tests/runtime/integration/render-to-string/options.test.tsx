...
test.each([null, 0, "ansi", "auto", "none", "TRUECOLOR"])(
  "rejects invalid color option %#",
  (color) => {
    expect(() => renderToString(Document, { color } as never)).toThrow(
      'renderToString option "color" must be a boolean, "ansi16", "ansi256", "truecolor", or undefined',
    );
  },
);
...
test("reads an accepted color accessor exactly once", () => {
  let reads = 0;
  const options = Object.defineProperty({}, "color", {
    enumerable: true,
    get() {
      reads++;
      return "ansi16";
