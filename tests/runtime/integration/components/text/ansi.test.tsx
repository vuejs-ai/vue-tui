...
// One SGR state machine reads the whole stream.
...
test("a nested Text closes only the channels its props set", () => {
  ...
  // Red continues after the child, which only sets bold. The reset in this
  // expectation is generated by the frame encoder, not authored content.
...
test("content re-asserting the colour already open still resolves it", () => {
  ...
  // The child opens blue, its content opens red, and leaving it closes the
  // foreground channel. The following authored red styles c.
...
test.each(["inline", "fullscreen"] as const)(
  "%s preserves nested Text style boundaries across grapheme updates",
  async (mode) => {
    const content = shallowRef("X");
    const App = defineComponent(() => () => (
      <Text>
        {`${ESC}[31mA`}
        <Text color="default">{content.value}</Text>
        {`b${ESC}[39m`}
      </Text>
    ));
    const result = await render(App, { mode, columns: 20, rows: 4, color: "truecolor" });
    expect(result.lastFrame()).toBe(`${ESC}[31mA${ESC}[39mXb`);

    for (const [text, expected] of [
      ["\u0301", `${ESC}[31mA\u0301${ESC}[39mb`],
      ["", `${ESC}[31mAb${ESC}[39m`],
      [`${ESC}[2J`, `${ESC}[31mA${ESC}[39mb`],
    ] as const) {
      content.value = text;
      await result.waitUntilRenderFlush();
      expect(result.lastFrame()).toBe(expected);
      expect(renderToString(App, { width: 20, color: "truecolor" })).toBe(expected);
    }
  },
);
