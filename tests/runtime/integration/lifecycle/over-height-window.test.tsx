...
test("mounted and string documents honor authored shrinking within the page", async () => {
  ...
  const expected = [...Array.from({ length: 23 }, (_, index) => `row ${index + 1}`), "FOOTER"].join(
    "\n",
  );
  expect(renderToString(Page)).toBe(expected);
  const result = await render(Page, { stdout: "stream" });
  try {
