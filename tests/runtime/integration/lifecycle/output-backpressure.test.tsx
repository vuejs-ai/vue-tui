...
test("an Inline transaction never writes again between write(false) and drain", async () => {
...
test("coordinated writes distinguish accepted backpressure from non-acceptance", async () => {
...
test("synchronized-output ownership follows physical BSU and ESU handoff", async () => {
  const writes: string[] = [];
...
test("normal unmount waits for drain before final output and restoration", async () => {
  let write!: (data: string) => CoordinatedWriteResult;
  const Root = defineComponent(() => {
    write = useStdout().write;
...
test("Fullscreen managed input waits through initial output backpressure without losing its route", async () => {
  const inputs: string[] = [];
  const Root = defineComponent(() => {
    useInput((event) => {
...
test("Inline Kitty and paste activation reconcile one handed control write at a time", async () => {
  const inputs: string[] = [];
  const Root = defineComponent(() => {
    useInput((event) => {
      inputs.push(inputLabel(event));
...
test("a semantic-input release during public output backpressure restores paste after drain", async () => {
...
test("blocked semantic-input changes retain only the final active state", async () => {
  const active = shallowRef(true);
  const inputs: string[] = [];
  let write!: (data: string) => CoordinatedWriteResult;
  const Root = defineComponent(() => {
