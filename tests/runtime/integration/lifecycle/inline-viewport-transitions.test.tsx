...
it("#450: non-TTY full-height rerenders should never clear terminal", async () => {
  ...
  app.mount(createInternalMountOptions({ stdout, stdin, stderr }));
  await nextTick();
  await nextTick();
...
it("#450: non-TTY grow-to-overflow rerender should not clear terminal", async () => {
  ...
  const fixturePath = new URL("./fixtures/inline-grow-to-overflow-rerender.tsx", import.meta.url)
    .pathname;
  ...
  const output = await new Promise<string>((resolve, reject) => {
    ...
    delete childEnvironment.NODE_DISABLE_COLORS;
    delete childEnvironment.FORCE_COLOR;
    delete childEnvironment.NODE_NO_WARNINGS;
