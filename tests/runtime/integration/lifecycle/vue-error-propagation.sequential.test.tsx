// Sequential: these tests replace the process-global console.error function.
...
test.sequential("a partial initial mount restores terminal state and allows remount", async () => {
  ...
  // Deliberately aligned with Vue: app.mount() threw, so Vue never took
  // container ownership and runs no cleanup of its own for this tree. Runtime
  // matches that instead of manufacturing the missing ownership link out of
  // Vue-private state, so no component scope is disposed. Everything Runtime
  // owns is still restored — the terminal modes and output checked below, and
  // the Yoga allocations proven in yoga-lifecycle.sequential.test.tsx.
...
// Companion to the partial-mount test above, covering the other failure shape:
// there the root mounted and a child's setup() threw, here the root's own
// setup() throws before any subtree exists. Both align with plain Vue, which
// runs no cleanup for either shape when the initial mount fails (verified
// against vue 3.4.38 and 3.5.34 with a stock custom renderer). Runtime restores
// what it owns regardless.
test.sequential("a root setup throw restores Runtime state without a Vue unmount", async () => {
  const originalError = new Error("root setup failure");
  const disposed: string[] = [];
  const stdout = makeFakeWritable({ columns: 80, rows: 24 });
  const stderr = makeFakeWritable({ columns: 80, rows: 24 });
  const writes = captureWrites(stdout);
  ...
  let thrown: unknown;
  try {
    app.mount({ stdout, stderr, stdin, mode: "fullscreen", patchConsole: false });
  } catch (error) {
    thrown = error;
  }
  if (thrown === undefined) app.unmount();

  // The original value still reaches both the caller and the exit barrier.
  expect(thrown).toBe(originalError);
  await expect(app.waitUntilExit()).rejects.toBe(originalError);

  // The documented gap: Vue never made this root unmountable, so its own scope
  // cleanup does not run. Everything Runtime owns is still restored below.
  expect(disposed).toEqual([]);

  expect(rawModeCalls.lastIndexOf(false)).toBeGreaterThanOrEqual(rawModeCalls.indexOf(true));
  const output = writes.join("");
  if (output.includes(ansiEscapes.enterAlternativeScreen)) {
    expect(output).toContain(ansiEscapes.exitAlternativeScreen);
  }

  const replacement = createApp(defineComponent(() => () => <Text>replacement</Text>));
  const { stream: replacementStdin } = makeFakeStdin();
  replacement.mount({ stdout, stderr, stdin: replacementStdin, patchConsole: false });
  replacement.unmount();
  await expect(replacement.waitUntilExit()).resolves.toBeUndefined();
});
