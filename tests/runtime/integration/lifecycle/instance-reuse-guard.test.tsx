...
test("color is snapshotted once before a mount attempt is consumed", async () => {
  const stdout = makeFakeWritable();
  const stderr = makeFakeWritable();
  const { stream: stdin } = makeFakeStdin();
  const app = createApp(App);
  let reads = 0;
  const options = Object.defineProperty({ stdout, stderr, stdin, patchConsole: false }, "color", {
    enumerable: true,
    get() {
      reads++;
      return reads === 1 ? "ansi16" : "invalid";
    },
  });
